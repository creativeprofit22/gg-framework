...
function isCustomForkCheckout(): boolean {
  ...
  return Boolean(
    origin?.includes("creativeprofit22/gg-framework") || branch === "custom/local-customizations",
  );
}
...
function buildEnvDefines(): Record<string, string> {
  ...
  const gitShortSha = env.VITE_GG_GIT_SHA ?? git(["rev-parse", "--short", "HEAD"]);
  ...
  const shortSha = env.VITE_GG_GIT_SHA ?? (detected ? gitShortSha : undefined);
