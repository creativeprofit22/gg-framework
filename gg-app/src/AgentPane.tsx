...
export function AgentPane({
  ...
  const hydrate = useCallback(async (): Promise<void> => {
    ...
    const isCurrent = (): boolean =>
      !disposedRef.current && generation === hydrateGenerationRef.current;
    ...
    try {
      ...
      // Reveal only the generation that still owns this pane target.
  ...
  const addNativeDroppedFiles = useCallback(async (paths: string[]): Promise<void> => {
    if (paths.length === 0) return;
  ...
  }, [addNativeDroppedFiles, insertDroppedFolderPaths, paneId, registerInput]);
  ...
  return (
    <div
      className={`app agent-pane${focused ? " pane-focused focused" : ""}${isFileDragOver ? " app-file-dragover" : ""}${windowFocused ? " window-focused" : ""}`}
    ...
    >
      <PaneHeader
        title={sessionTitle ?? PRODUCT_DISPLAY_NAME}
        collapsed={navHidden}
        onToggle={toggleNav}
        usage={<TitleUsageMeter currentProvider={state?.provider ?? ""} />}
      >
        ...
        <div className="img-grid">
          {item.images.map((img, i) => {
            const openImage = (): void => {
              if (img.path) void onOpenProjectPath(img.path);
            };
            return (
              <figure
                key={img.path ?? i}
