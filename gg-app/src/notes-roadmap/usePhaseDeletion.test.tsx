...
const phase: NotesPhase = {
  id: "phase",
  title: "A very long phase title ".repeat(8),
  goal: "Preserve data",
  doneWhen: [],
  order: 0,
  status: "not-started",
...
const snapshot = (): ProjectNotesSnapshot => ({
  projectKey: "synthetic",
  revision: 1,
  document: { ...createEmptyNotesDocument(now), phases: [structuredClone(phase)] },
});
function fixture() {
  let current = snapshot();
  const bridge: PhaseDeletionBridge = {
    prepare: vi.fn(async () => current),
    mutate: vi.fn(async (request): Promise<PhaseDeletionOutcome> => {
      current = {
        ...current,
        revision: current.revision + 1,
        document: {
          ...current.document,
          phases: [applyNotesPhaseDeletion(current.document.phases[0]!, request, now)],
        },
      };
      return {
        status: "committed",
...
describe("phase deletion controller", () => {
  ...
  it("keeps exact request identity through uncertain retry and supports durable Undo", async () => {
    const f = fixture();
    const mutate = vi.mocked(f.bridge.mutate);
    ...
    expect(hook.result.current.request).toBe(input);
    await act(async () => {
      await hook.result.current.confirm();
    });
  ...
  it("never rebases a stale destructive request without renewed confirmation", async () => {
    ...
    expect(hook.result.current.request).toBeNull();
    expect(hook.result.current.error).toContain("confirm again");
    await act(async () => {
      await hook.result.current.confirm();
    });
    expect(f.bridge.mutate).toHaveBeenCalledTimes(1);
  ...
  it("prevents double dispatch and ignores a late reply after switching projects", async () => {
    const f = fixture();
    let finish!: (value: PhaseDeletionOutcome) => void;
    vi.mocked(f.bridge.mutate).mockImplementation(
      () =>
        new Promise((resolve) => {
          finish = resolve;
        }),
    );
    const hook = renderHook(({ project }) => usePhaseDeletion(project, f.bridge), {
      initialProps: { project: "synthetic" },
    });
    await act(async () => {
      await hook.result.current.begin(phase, "delete");
    });
  ...
  it("preserves context after failed saves and refuses Undo for a different deletion", async () => {
    const f = fixture();
    vi.mocked(f.bridge.prepare).mockRejectedValueOnce(new Error("Unsaved edits"));
    const hook = renderHook(() => usePhaseDeletion("synthetic", f.bridge));
    await act(async () => {
      await hook.result.current.begin(phase, "delete");
    });
    expect(hook.result.current.error).toBe("Unsaved edits");
    expect(hook.result.current.target?.phase.id).toBe(phase.id);
    expect(f.bridge.mutate).not.toHaveBeenCalled();
    await act(async () => {
      await hook.result.current.begin(phase, "recover", "different-deletion");
    });
    expect(hook.result.current.request).toBeNull();
  });
});
...
it("uses Cancel-first focus and Escape closes only the nested confirmation", async () => {
  ...
  function Harness() {
    ...
    return (
      <Modal title="Notes" onClose={outerClose}>
        <button onClick={() => void controller.begin(phase, "delete")}>Delete fixture</button>
        <NotesPhaseDeletionDialog controller={controller} />
      </Modal>
    );
  }
  render(<Harness />);
  ...
  expect(screen.queryByText("Delete phase")).toBeNull();
  expect(f.bridge.mutate).not.toHaveBeenCalled();
});
...
it("shows retained deletion date, placement and recovery after remount", () => {
  ...
  const deleted = applyNotesPhaseDeletion(phase, request, now);
