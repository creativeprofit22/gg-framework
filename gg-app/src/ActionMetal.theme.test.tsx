...
vi.mock("metal-fx", () => ({
  MetalFx: ({ theme, children }: { theme: string; children: React.ReactNode }) => (
    <div className="action-metal" data-theme={theme}>
      {children}
    </div>
  ),
}));
...
it("passes the saved theme to the shader without replacing its host", async () => {
  ...
  await waitFor(() =>
    expect(container.querySelector(".action-metal")?.getAttribute("data-theme")).toBe("dark"),
  );
