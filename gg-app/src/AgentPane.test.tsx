...
describe("Qwen Cloud thinking labels", () => {
  it.each([
    ...
    vi.mocked(pane.cycleThinking).mockResolvedValue({
      thinkingLevel: "high",
      supportedThinkingLevels: ["high", "max"],
    });
...
describe("pane-local opening (mocked native transport)", () => {
  ...
  it("restores prompt text, the last reference chips, and recall when the prompt starts with an example block", async () => {
    const pane = client("pane-reference-restore", 1);
    const text = "Referenced files:\n- earlier.ts\n\nPlease inspect this example.";
    vi.mocked(pane.listHistory).mockResolvedValue([
      { role: "user", text: `${text}\n\nReferenced files:\n- src/current.ts` },
    ]);
    ...
    expect(chips[0].getAttribute("title")).toBe("src/current.ts");
    expect(chips[0].textContent).toBe("src/current.ts");
    const input = screen.getByRole("textbox") as HTMLTextAreaElement;
    fireEvent.keyDown(input, { key: "ArrowUp" });
...
describe("queued cancellation ordering (mocked native transport)", () => {
  it.each([false, true])(
    "ignores a delayed cancellation snapshot after newer events (reset=%s)",
    async (reset) => {
      nativeMocks.realMentor = true;
      const pane = client("queue-cancel", 1);
      const emit = liveEvents(pane);
      const pending = deferred<boolean | null>();
      pane.cancelQueued = vi.fn(() => pending.promise);
      vi.mocked(pane.getState).mockResolvedValue({
        ...agentState("azure:gpt-test"),
        running: true,
        runState: "running",
      ...
      act(() => emit("queued", { count: 1, messages: [{ id: "q1", text: "older" }] }));
      ...
      act(() => {
        if (reset) emit("session_reset", {});
        emit("queued", { count: 1, messages: [{ id: "q2", text: "newer" }] });
      });
      // Deliberately retain the legacy payload: even this stale response must never own UI state.
      await act(async () => pending.resolve([] as unknown as boolean));
      expect(screen.getByTitle("newer")).toBeTruthy();
...
describe("command refresh ordering (mocked native transport)", () => {
  it("does not apply a previous client's catalog after switching sessions", async () => {
    ...
    const nextPane = client("new-command-owner", 2);
    const pending = deferred<AgentModule.SlashCommand[] | null>();
...
describe("AgentPane lifecycle", () => {
    ...
    async ({ busy, text }) => {
      await withRealSidecar(async ({ project, manager, open, request, generation }) => {
        ...
        try {
          ...
          expect((input as HTMLTextAreaElement).value).toBe(text);
          expect(screen.getByRole("button", { name: "Remove file.txt" })).toBeTruthy();
          // A generic 500 is not an authoritative rejection receipt at the native boundary.
          expect(view.container.querySelectorAll(".user-msg")).toHaveLength(busy ? 0 : 1);
          expect(pane.sendPrompt).toHaveBeenCalledOnce();
        } finally {
          cleanup();
        }
  ...
  it("blocks setup assessment in Plan mode but keeps report and detail access (mocked native IPC)", async () => {
    ...
    await waitFor(() =>
      expect(
        (screen.getByRole("button", { name: "Refresh results" }) as HTMLButtonElement).disabled,
      ).toBe(false),
    );
    ...
    await waitFor(() =>
      expect(pane.programmatic).toHaveBeenCalledExactlyOnceWith({
        version: 1,
        action: "report",
        offset: 0,
      }),
      ...
      if (request.action === "inspect-setup")
        return {
          ...
          assessment: {
            ...
            limitations: ["Settings cannot be proposed or saved."],
            coverage: [],
            observations: [],
  ...
  it.each(["unavailable", "failed", "cancelled"] as const)(
    "supersedes button success with slash %s status and counts (mocked native IPC)",
    async (status) => {
      nativeMocks.realMentor = true;
      ...
      const success = {
        ...
        summary: "Previous button assessment",
        limitations: [],
        observations: [],
        coverage: [],
      ...
      const assessment = {
        ...
        summary: "Latest slash assessment",
        deterministic: { status, reason: "Latest host outcome" },
      };
  ...
  it.each(["success", "transport-lost"] as const)(
    ...
    async (outcome) => {
      ...
      vi.mocked(pane.programmatic).mockClear();
      vi.mocked(pane.programmatic).mockImplementation(async (request) =>
        request.action === "discover" ? receipt.promise : report,
      );
      render(<AgentPane client={pane} target={target} />);
      await waitFor(() => expect(pane.listCommands).toHaveBeenCalled());
      fireEvent.click(screen.getByRole("button", { name: "Opportunities" }));
      ...
      const request = vi
        .mocked(pane.programmatic)
        .mock.calls.find(([request]) => request.action === "discover")![0];
      ...
      if (outcome === "transport-lost") {
        ...
        const candidate = {
          ...
          outcome: "Inspect recurring records",
          rationale: "Repeated",
          uncertainty: "Scoped",
          workflow: {
            trigger: "Change",
            ...
            currentProcess: [],
            output: "Report",
            successCheck: "Find issue",
            scope: "Project",
            mutationBoundary: "Read-only",
            repeatability: { basis: "inferred" as const, explanation: "Recurring" },
          },
          evidence: [],
          alternatives: [],
          risks: [],
  ...
  it.each(["before-run-end", "after-run-end", "after-response"] as const)(
    "keeps exact setup when assessment completion is %s (mocked native IPC)",
    async (ordering) => {
      nativeMocks.realMentor = true;
      ...
      await act(async () =>
        receipt.resolve({
          ...
          assessment: {
            ...
            mode: "setup",
            status: "unavailable",
            summary: "Model unavailable; settings can still be reviewed.",
            limitations: ["No model assessment was completed."],
            coverage: [],
            observations: [],
            deterministic: { status: "not-run", reason: "setup" },
          },
          proposal: {
            ...
            operation: "initial",
            fingerprint: hash,
            profileJson: "exact reviewed settings",
            configuration: {
              status: "missing",
              currentFingerprint: hash,
              refreshAvailable: false,
  ...
  it.each([
    ...
    fireEvent.click(screen.getByRole("button", { name: "Send message" }));
    expect(await screen.findByText("Command input blocked")).toBeTruthy();
    expect(screen.getByText(/optional focus of at most 4,000 characters/)).toBeTruthy();
    expect(screen.getByText(/newlines and tabs are allowed/)).toBeTruthy();
    expect((input as HTMLTextAreaElement).value).toBe(draft);
    expect(pane.sendPrompt).not.toHaveBeenCalled();
    expect(pane.programmatic).not.toHaveBeenCalled();
  });
  ...
  it.each([
    ...
    fireEvent.change(input, { target: { value: draft } });
    fireEvent.click(screen.getByRole("button", { name: "Send message" }));
    await waitFor(() => expect(pane.sendPrompt).toHaveBeenCalledWith(draft.trim(), [], undefined));
    expect(screen.queryByText("Command input blocked")).toBeNull();
  });

  it.each([
    ["ordinary chat", "code", "hello", "built-in"],
    ["custom command", "code", "/programmatic", "custom"],
    ["non-code custom command", "chat", "/programmatic", "custom"],
    ...
    const input = await screen.findByRole("textbox");
    await waitFor(() => expect(pane.listCommands).toHaveBeenCalled());
    const draft = `${prefix} ${"x".repeat(4_001)}\u0000`;
    fireEvent.change(input, { target: { value: draft } });
    fireEvent.click(screen.getByRole("button", { name: "Send message" }));
    await waitFor(() => expect(pane.sendPrompt).toHaveBeenCalledWith(draft, [], undefined));
    expect(screen.queryByText("Command input blocked")).toBeNull();
  ...
  it("accepts CR/LF/tab focus and rejects attachments and references with optional focus", () => {
    ...
    expect(noInputSlashSubmissionError(draft, commands, 1, 0)).toContain("attachments");
    expect(noInputSlashSubmissionError(draft, commands, 0, 1)).toContain("file references");
  });
