import type { ProgrammaticAssessment, ProgrammaticAssessmentEvent, ProgrammaticAssessmentIdentity } from "@kenkaiiii/gg-core/programmatic-assessment-contract";
import type { ProgrammaticChatResponse } from "@kenkaiiii/gg-core/programmatic-chat-contract";
import type { DiscoveryCandidate, DiscoveryProjection, DiscoveryReview, RecommendationHistoryReport, RecommendationDetail } from "@kenkaiiii/gg-core/programmatic-recommendation-contract";
import type { ProgrammaticChatState } from "./programmatic-chat-state";

export type ProgrammaticSelection = { source: "deterministic"; id: string } | { source: "current" | "history"; id: string };
...
export function assessmentEvent(state: ProgrammaticChatState, update: ProgrammaticAssessmentEvent): ProgrammaticChatState {
  ...
  return { ...state, assessmentSequence: update.sequence, assessmentIdentity: update,
    assessmentUncertain: false, assessmentPending: update.phase === "started",
    ...(update.phase === "completed" ? assessmentDisplay(state, update.assessment) : { assessmentRetained: !!state.assessment, discoveryStale: true, candidateStale: !!state.candidateDetail }) };
}
const sameOwner = (a: { sessionId: string; conversationId: string }, b: { sessionId: string; conversationId: string }) =>
...
export function assessmentInterrupted(state: ProgrammaticChatState): Partial<ProgrammaticChatState> {
  const request = state.assessmentRequest, identity = state.assessmentIdentity;
  return state.assessmentPending && request && identity && sameOwner(request, identity) &&
    request.requestId === identity.requestId
    ? { assessmentPending: false, assessmentUncertain: true, discoveryStale: true, candidateStale: !!state.candidateDetail } : {};
}
...
export function assessmentResponse(state: ProgrammaticChatState, response: ProgrammaticChatResponse): Partial<ProgrammaticChatState> {
  if (!("assessment" in response) || !response.assessment) return {};
  const assessment = response.assessment, receipt = assessment.lifecycle, request = state.assessmentRequest;
  if (!receipt) return state.assessmentSequence > state.assessmentRequestSequence ? {} : assessmentDisplay(state, assessment);
  if (response.action !== state.operation || !request || !sameOwner(request, receipt) || receipt.requestId !== request.requestId ||
    receipt.sequence <= state.assessmentRequestSequence || receipt.sequence < state.assessmentSequence) return {};
  if (receipt.sequence === state.assessmentSequence &&
    ((!state.assessmentPending && !state.assessmentUncertain) || !state.assessmentIdentity ||
      !sameOwner(receipt, state.assessmentIdentity) || receipt.requestId !== state.assessmentIdentity.requestId)) return {};
  return { ...assessmentDisplay(state, assessment), assessmentSequence: receipt.sequence, assessmentIdentity: receipt,
    assessmentPending: false, assessmentUncertain: false };
}
...
export function selectCandidate(state: ProgrammaticChatState, source: "current" | "history", id: string): ProgrammaticChatState {
  const candidate = source === "current" ? state.discovery?.candidates.find((item) => item.candidateId === id)
    : state.historyDetail?.candidate.id === id ? state.historyDetail.discovery : undefined;
  return { ...state, selection: { source, id }, candidateDetail: candidate ?? null, candidateReview: null,
    candidateStale: source === "history" || state.discoveryStale || state.assessmentPending || state.assessment?.status !== "completed" ||
      !state.assessment.discovery?.candidates.some((item) => item.candidateId === id) };
}
type DiscoveryResponse = Extract<ProgrammaticChatResponse, { ok: true; action: "review-candidate" | "history-report" | "history-detail" |
  "history-inspect-decision" | "history-inspect-correspondence" | "history-apply" }>;
export function discoveryResponse(state: ProgrammaticChatState, response: DiscoveryResponse): ProgrammaticChatState {
  switch (response.action) {
    case "review-candidate": {
      const candidate = response.candidateReview.candidate;
      if (state.selection?.source !== "current" || candidate.candidateId !== state.selection.id || candidate.assessmentId !== state.candidateDetail?.assessmentId ||
    ...
    case "history-apply": return { ...state, candidateStale: true,
      notice: response.changed ? "History decision saved. Reload the selected historical detail." : "No history decision changed." };
  }
}
