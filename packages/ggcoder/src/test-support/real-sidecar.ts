...
export async function withRealSidecar<T>(run: (fixture: {
...
}) => Promise<T>, options: { parkQuestion?: boolean; queueDrain?: "steering" | "stranded"; qwenRuntimeKey?: string; progressXp?: number; truncated?: "refusal" | "empty_response" } = {}): Promise<T> {
  ...
  try {
    ...
    Object.assign(env, {
      ...
      GG_FIXTURE_TRUNCATED: options.truncated ?? "",
      GG_FIXTURE_PROGRESS_XP: options.progressXp === undefined ? "" : String(options.progressXp),
