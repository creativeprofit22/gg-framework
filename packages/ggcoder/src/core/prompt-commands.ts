...
Build all six bytes in memory, validate the complete destination set, then commit the set with rollback-safe temporary files. Skip writes whose bytes already match. Use stable key ordering, stable path ordering, LF endings, no timestamps, no absolute machine paths, and no shell command strings. A same-evidence rerun must render byte-identical support files and leave a clean second git status. Changed evidence triggers one complete rerender; changed generated bytes trigger a conflict, never overwrite.
