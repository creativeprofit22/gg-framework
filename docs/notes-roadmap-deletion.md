...
Execution admission, cancellation, advancement discovery, model-facing projections, active phase context, reminder reservation/final claims and repository mutations reject deleted phases. Reservation and claim are not the last gate: a claim can commit durably before deletion and still resolve afterwards, so the delivery host in [`roadmap-reminders.ts`](../gg-app/src/roadmap-reminders.ts) rechecks final admission against the latest adopted document — phase presence, deletion generation, occurrence, disposal and project identity — after every awaited claim and before any in-app or native dispatch, and `ProjectNotes.tsx` revalidates late callbacks before queueing them. Ordering is explicit: the webview host owns the deletion gate; the native command in `lib.rs` only enforces complete scoped phase/occurrence identity plus at-most-once dispatch per occurrence. The occurrence stays consumed when admission rejects, recovery never restores reminders, and notifications already handed to the OS are never retracted. Recovered phases reject stale revisions/sessions, retired implementation receipts and sessionless legacy lifecycle callbacks. Activity/history arrays remain intact; retired evidence is not presented as current verification in a new run.
