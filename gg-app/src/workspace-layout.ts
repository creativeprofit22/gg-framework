...
function parseAgent(value: unknown, requireKind: boolean): WorkspacePaneValue | undefined {
  ...
  if (
    ...
    return undefined;
  const allowed = record.kind === "agent" ? ["kind", "cwd", "sessionPath"] : ["cwd", "sessionPath"];
  ...
  for (const id of ids) {
    ...
    if (
      ...
      if (
        ...
        !parseTarget(terminal)
      )
        return null;
      terminalIds.add(id);
    } else {
      const agent = parseAgent(raw, true);
...
function migrateV0ToV6(record: Record<string, unknown>): WorkspaceLayout | null {
  ...
  if (record.version === 6) {
    ...
    if (record.version === 0) {
      primary = parseTarget(record.primary ?? null, true);
      secondary = parseTarget(record.secondary ?? null, true);
    } else {
      if (typeof record.panes !== "object" || record.panes === null) return null;
      const p = record.panes as Record<string, unknown>;
      primary = parseTarget(p.primary);
      secondary = parseTarget(p.secondary);
    }
  ...
  return {
    ...
    panes: Object.fromEntries(
      Object.entries(layout.panes).map(([id, pane]) => [
        id,
        pane ? { kind: "agent", cwd: pane.cwd, sessionPath: pane.sessionPath } : null,
      ]),
    ),
  };
}
