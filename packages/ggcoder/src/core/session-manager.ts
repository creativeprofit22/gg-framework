...
export class SessionManager {
  ...
  async load(
    sessionPath: string,
    expected?: { projectKey?: string; resolveCanonical?: boolean },
  ): Promise<{
    header: SessionHeader;
    entries: SessionEntry[];
    path: string;
  }> {
    // Normal resume resolves to the conversation tip. Exact checkpoint callers
    // can opt out while retaining archive thawing and project validation.
    const selectedPath =
      expected?.resolveCanonical === false
        ? sessionPath
        : ((await this.resolveCanonicalSession(sessionPath)) ?? sessionPath);
    const effectivePath = await thawSessionArchive(selectedPath);
    const loaded = await this.loadPhysicalCheckpoint(effectivePath);
  ...
  /**
   * Walk entries in file order, tracking how many branch messages have been
   * written so far, and hand each custom entry that count.
   */
  private mapCustomEntriesInFileOrder<T>(
    ...
    const onBranch = leafId
      ? new Set(this.getBranch(entries, leafId).map((entry) => entry.id))
      : new Set(entries.map((entry) => entry.id));
    const output: T[] = [];
    let messagesSoFar = 0;
    for (const entry of entries) {
      if (entry.type === "message") {
        if (onBranch.has(entry.id) && entry.message.role !== "system") messagesSoFar += 1;
        continue;
      }
      if (entry.type !== "custom") continue;
      output.push(...project(entry, messagesSoFar));
    }
    return output;
  }

  /** Return the last valid durable active-phase record. */
