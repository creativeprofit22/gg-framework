...
it.each([false, true])(
  "keeps evidence coverage independent from completed empty freshness (inspected=%s)",
  (inspected) => {
    const value = emptyDiscoveryAssessment(inspected);
    expect(isProgrammaticAssessment(value)).toBe(true);
    const state = programmaticChatReducer(
    ...
    const started = programmaticChatReducer(state, {
      type: "assessment",
      generation: "one",
      event: {
        conversationId: "chat",
...
const candidate: DiscoveryCandidate = {
  ...
  workflow: {
    ...
    repeatability: { basis: "inferred", explanation: "Recurring records" },
  },
  evidence: [],
  alternatives: [],
  risks: [],
...
const completion = (value: ProgrammaticAssessment, sequence: number) => ({
...
function selected() {
  let state = programmaticChatReducer(
    initialProgrammaticChatState("one"),
    completion(assessment, 1),
  );
...
it("keeps candidate identity independent from scanner selection and execution", () => {
  ...
  expect(programmaticChatReducer(state, { type: "select", id: "scanner" }).candidateDetail).toBe(
    candidate,
  );
});
it.each(["cancelled", "unavailable", "incomplete", "completed"] as const)(
  "preserves previous candidate through refresh ending %s",
  (status) => {
    const original = selected();
    const started = programmaticChatReducer(original, {
      type: "assessment",
      generation: "one",
      event: {
        conversationId: "chat",
    ...
    const next = programmaticChatReducer(
      started,
      completion(
        {
          ...assessment,
          status,
          discovery: { assessmentId: candidate.assessmentId, candidates: [] },
        },
        2,
      ),
    );
    ...
    expect(
      programmaticChatReducer(next, { type: "reset", generation: "two" }).candidateDetail,
    ).toBeNull();
  },
);
it.each(["cancelled", "unavailable", "incomplete", "completed"] as const)(
  "accepts a current %s response without discovery independently of retained candidates",
  (status) => {
    ...
    const result = programmaticChatReducer(started, {
      type: "response",
      generation: "one",
      epoch: 1,
...
it("does not turn a selected historical display or delayed review response into current authority", () => {
  const state = programmaticChatReducer(selected(), {
    type: "select-candidate",
    source: "history",
    id: candidate.candidateId,
  });
  expect(canReviewProgrammaticCandidate(state)).toBe(false);
  const late = programmaticChatReducer(state, {
    type: "response",
    generation: "one",
    epoch: state.epoch,
    response: {
      ...
      ok: true,
      candidateReview: { status: "prepared", candidate, summary: "Old review" },
    },
  });
...
it("does not revive previous evidence from a delayed response while a newer assessment is starting", () => {
  let state = programmaticChatReducer(selected(), {
    type: "start",
    generation: "one",
    epoch: 1,
    operation: "discover",
...
function loadedHistory() {
  ...
  return {
    ...
    historyReport,
    historyDetail,
    selection: { source: "history" as const, id: summary.id },
    candidateStale: true,
  };
}
...
const savedAssessment = (history: ProgrammaticAssessment["history"]): ProgrammaticAssessment => ({
  ...
  lifecycle: { ...request, sequence: 2 },
  ...(history ? { history } : {}),
});
function historyCompletion(
  state: ProgrammaticChatState,
  value: ProgrammaticAssessment,
  path: "event" | "response",
) {
  ...
  return programmaticChatReducer(
    state,
    path === "event"
      ? { type: "assessment", generation: "one", event }
      : { type: "response", generation: "one", epoch: state.epoch, response },
  );
}
it.each(["event", "response"] as const)(
  "invalidates loaded report and selected detail through %s completion",
  (path) => {
    ...
    const result = historyCompletion(
      state,
      savedAssessment({
        status: "saved",
...
it("rejects stale and foreign completions without changing history freshness", () => {
  ...
  const state = {
    ...
    assessmentIdentity: { ...request, sequence: 3 },
    assessmentRequest: request,
    assessmentRequestSequence: 2,
    epoch: 2,
    operation: "discover" as const,
  };
  const value = savedAssessment({
    status: "saved",
    assessmentId: candidate.assessmentId,
    historyRevision: 9,
  });
  for (const patch of [
    { sequence: 2 },
    { sequence: 4, sessionId: "foreign" },
    { sequence: 4, conversationId: "foreign" },
  ]) {
    const identity = { ...request, ...patch };
    const event = {
      ...identity,
      phase: "completed" as const,
      assessment: { ...value, lifecycle: identity },
    };
    expect(isProgrammaticAssessmentEvent(event)).toBe(true);
    expect(programmaticChatReducer(state, { type: "assessment", generation: "one", event })).toBe(
      state,
    );
  }
  const response = {
  ...
  const foreign = programmaticChatReducer(state, {
    type: "response",
    generation: "one",
    epoch: 2,
    response: {
      ...response,
      assessment: { ...value, lifecycle: { ...request, sessionId: "foreign", sequence: 4 } },
    },
  });
...
it.each([0, 1])("does not invalidate history for same/older revision %i", (historyRevision) => {
  const result = historyCompletion(
    loadedHistory(),
    savedAssessment({ status: "saved", assessmentId: candidate.assessmentId, historyRevision }),
    "event",
  );
  expect(result.historyReportStale).toBe(false);
  expect(result.historyDetailStale).toBe(false);
});
it.each([undefined, "disabled", "unsaved", "acknowledgement-unknown"] as const)(
  "handles %s history saving conservatively",
  (status) => {
...
function readHistory(
  state: ReturnType<typeof initialProgrammaticChatState>,
...
it("clears each stale view only on a fresh read at the acknowledged revision", () => {
  const loaded = loadedHistory();
  let state = historyCompletion(
    loaded,
    savedAssessment({ status: "saved", assessmentId: candidate.assessmentId, historyRevision: 2 }),
    "event",
  );
  state = readHistory(state, {
    version: 1,
    ok: true,
    action: "history-report",
    report: loaded.historyReport,
  });
  ...
  state = readHistory(state, {
    ...
    ok: true,
    action: "history-report",
    report: { ...loaded.historyReport, revision: 2 },
  });
  ...
  state = readHistory(state, {
    ...
    detail: { ...loaded.historyDetail, historyRevision: 2 },
  });
...
it.each([true, false])(
  "does not let an in-flight read settle an uncertain save (already loaded: %s)",
  (alreadyLoaded) => {
    const loaded = loadedHistory();
    const started = {
      ...loaded,
      historyReport: alreadyLoaded ? loaded.historyReport : null,
      epoch: 1,
      operation: "history-report" as const,
    };
    let state = historyCompletion(
      started,
      savedAssessment({
    ...
    const response = {
      version: 1 as const,
      ok: true as const,
      action: "history-report" as const,
      report: loaded.historyReport,
    };
    state = programmaticChatReducer(state, {
      type: "response",
...
function pendingRequest() {
  ...
  return programmaticChatReducer(started, {
    type: "assessment",
    generation: "one",
    event: { ...request, sequence: 2, phase: "started" },
  });
}
const receipt = (
  lifecycle = { ...request, sequence: 2 },
  status: ProgrammaticAssessment["status"] = "completed",
) => ({
  ...
  response: {
    ...
    action: "discover" as const,
    ok: true as const,
    assessment: { ...assessment, status, lifecycle },
  },
});
it.each(["started", "completed"] as const)(
  "does not replace a genuinely newer %s assessment with an older receipt",
  (phase) => {
    const state = programmaticChatReducer(pendingRequest(), {
      type: "assessment",
      generation: "one",
      event:
        phase === "started"
          ...
          : {
              ...
              sequence: 3,
              phase,
              assessment: { ...assessment, summary: "Newer" },
            },
    });
...
it.each([{ sessionId: "other" }, { conversationId: "other" }, { requestId: "other" }])(
  "rejects mismatched receipt owner %j",
  (patch) => {
...
it.each(["cancelled", "unavailable", "incomplete"] as const)(
  "settles matching %s receipts without reviving previous detail",
  (status) => {
...
it("rejects old generations and epochs and does not regress an accepted completion", () => {
  ...
  const lateStart = programmaticChatReducer(settled, {
    type: "assessment",
    generation: "one",
    event: { ...request, sequence: 2, phase: "started" },
  });
...
it("marks matching transport loss as unknown and still accepts a late completion", () => {
  const state = pendingRequest();
  const failed = programmaticChatReducer(state, {
    type: "error",
    generation: "one",
    epoch: 1,
    error: "Cancelled transport",
    reconcile: false,
  });
  ...
  const completed = programmaticChatReducer(failed, {
    type: "assessment",
    generation: "one",
...
it("keeps selected context and disables handoff on unknown acknowledgement without replaying work", () => {
  ...
  const failed = programmaticChatReducer(started, {
    type: "error",
    generation: "one",
    epoch: 1,
    error: "Response lost",
