...
describe("useProjectNotes sidecar authority", () => {
  it.each(["load", "migration", "save"])(
    "blocks unsupported %s without local fallback writes",
    async (stage) => {
      ...
      const unsupported = {
        status: "unsupported",
        source: "primary",
        message: "Open with a compatible app; files were preserved.",
      } as const;
      ...
      await waitFor(() =>
        expect(hook.result.current.diagnostics.authority).toEqual([
  ...
  it("accepts genuine lower backup recovery after a fresh read confirms it", async () => {
    ...
    await act(async () =>
      opened.resolve({ status: "ok", snapshot: recovered, recoveredFromBackup: true }),
    );
    expect(client.getCalls).toBe(2);
  ...
  it.each(["backup read", "conflict"] as const)(
    ...
    async (response) => {
      ...
      const stale = {
        projectKey: cwd,
        revision: 4,
        document: { ...initial, reference: "backup 4" },
      };
