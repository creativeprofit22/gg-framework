...
const summary = "Your custom provider setup stays exactly how you like it because it keeps fork-specific sessions separate. You still get upstream's clearer recovery guidance, so updates are easier to follow without giving up your setup.";

function sourceSession() {
  return { getState: () => ({ provider: "anthropic" as const, model: "test-model", cwd: "/repo" }) };
}
...
function fakeSession(response = JSON.stringify({ version: 1, summary })) {
  ...
  const session: DecisionSummarySession = { initialize, prompt, dispose, getMessages: () => messages };
...
describe("decision summary validation", () => {
  ...
  it.each([
    ...
    [{ ...context, decisions: [{ ...context.decisions[0], files: Array(41).fill(context.decisions[0]!.files[0]) }] }],
  ...
  it("strictly parses the response envelope", () => {
    expect(parseDecisionSummaryResponse(JSON.stringify({ version: 1, summary }))).toEqual({ version: 1, summary });
  });
...
describe("AppSidecarDecisionSummaryService", () => {
  it("uses the active provider in a one-turn no-tools session", async () => {
    ...
    await expect(service.summarize(sourceSession(), context)).resolves.toEqual({ version: 1, summary });
    expect(options).toMatchObject({
      provider: "anthropic", model: "test-model", cwd: "/repo",
      systemPrompt: DECISION_SUMMARY_SYSTEM_PROMPT, transient: true, allowedTools: [],
      projectCustomization: false, globalSubagents: false, coderSlashCommands: false,
      selfCorrectionHooks: false, loadExtensions: false, orchestrationPrompt: false,
      mcpEnabled: false, maxTurns: 1, maxTurnExtensions: 0,
    });
