...
it("describes unavailability, prevents native activation and restores availability", () => {
  const onSwap = vi.fn();
  const props = { paneId: "a", label: "Conversation A", helpId: "help", buttonRef: vi.fn(), onSwap };
  const { rerender } = render(<PaneSwapButton {...props} unavailableReason="Swap unavailable while a pane is closing." />);
...
it("uses native button activation, blocks repeated keydowns but leaves normal Space keyup intact", () => {
  const onSwap = vi.fn(); const ref = vi.fn();
  render(<PaneSwapButton paneId="a" label="Conversation A" helpId="help" buttonRef={ref} onSwap={onSwap} />);
  ...
  expect(onSwap).not.toHaveBeenCalled();
  fireEvent.click(button); expect(onSwap).toHaveBeenCalledExactlyOnceWith("a", true, true);
  fireEvent.click(button, { detail: 1 });
