...
- **Implementation status:** Phases 00–25 are implementation-complete. Phase 26's local and isolated Windows gates pass, but its release hard stop remains open pending exact-reviewed-diff six-job CI and an isolated macOS `tauri dev` Notes smoke. Phase 25's visible packaged Windows toast remains deferred manual release QA and is not proven by the dev fixture.
...
- **2026-07-29 Phase 25 evidence:** Notes v3 now stores stable reminder IDs, per-occurrence keys, and repository-owned delivery attempts. One daemon-shared coordinator watches active project sessions, emits content-free due signals, applies focused grace, leases one occurrence to one authenticated pane, and persists one atomic claim before any in-app/native side effect. Focused app delivery uses one queued non-modal alert; background delivery lazily requests permission, sends fixed privacy-safe native copy, and asks the OS for at most one platform sound when the existing sound toggle is enabled. Later today, Tomorrow, custom local time, one-hour snooze, dismiss, Resume/Open phase, due badges, fallback recovery, restart hydration, and multi-window deduplication are covered. Focused gates passed 157 backend tests, 172 app tests, 114 Rust tests, both package checks, app lint/format, full package tests, root lint/format, both builds, sidecar bundling, generated audits, fixture smokes, and diff checks. The isolated dev UI acceptance rerun passed with one authenticated focused `in-app` claim, alert/detail screenshots, background reservation blocked, and `nativeToastAttempted: false`. Rendered desktop/360 px evidence and critique are under `.gg/screenshots/phase25-*` and `.gg/evidence/phase-25/`; a visible packaged Windows toast, macOS sound, native screen-reader speech, and field performance remain unverified.
- **2026-07-29 packaged-automation safety audit:** The generic Windows packaged runner, the Phase 25 packaged runner, and both normal package commands were retired because installed-app isolation and dev AppUserModelID isolation were not proven. Their scripts now contain no installer execution/extraction, app launch, host process enumeration/termination, shortcut or registry mutation, notification inspection, or cleanup flows. The earlier nonce packaged attempt did not produce a visible toast and is not release evidence. The isolated focused Phase 25 dev fixture is the supported automated verification path; it redirects profile/data roots, blocks background delivery, preserves real user data, and captured passing alert/detail evidence.
- **2026-07-29 Phase 26 release-gate audit:** Autopilot Ken reviewed HEAD `a5a0c3bcf624e7976d42d8589d26fe1a5629e419` plus the 46-entry `git status --short` working tree (34 tracked, 12 untracked). Fresh focused gates passed 249 backend and 255 app tests; full gates passed 2,213 ggcoder tests with 15 explicit skips, 601 app tests plus 8 workspace evidence tests, and 117 locked Rust tests. Check, lint, format, build, staged-runtime, bundled-sidecar smoke, generated audits, and diff checks pass. The isolated Windows dev UI fixture passed with one focused in-app claim and `nativeToastAttempted: false`. CI run [`30421549329`](https://github.com/creativeprofit22/gg-framework/actions/runs/30421549329) is green across six jobs for bare HEAD, but it predates the retained working-tree diff and is therefore not Phase 26 CI proof; no isolated macOS runner is available.
- **Next gate:** Commit/review the retained diff, obtain all six CI jobs for that exact commit, then run the isolated current-checkout macOS `tauri dev` Notes inspect/Resume smoke. Keep Phase 26 open until both pass.
...
- **Later-track audit:** Phases 15–25 are implementation-complete. Phase 26 has fresh local and Windows evidence but remains incomplete on exact-diff CI and isolated macOS desktop evidence. Visible packaged Windows toast delivery remains deferred manual release QA.
...
- `NotesDocumentV3` preserves every v2 Notes field and adds strict ordered phase and structured-reference collections at both the sidecar and app IPC/storage boundaries.
- Phase records carry stable identity, Done-when criteria, status, source prompt, reference links, typed session ID/path, reminders, attention, timestamps, manual override markers, and source/timestamp lifecycle events. Reference records carry canonical repository/source metadata and exact optional revision/path/range/issue/PR/query/anchor coordinates.
- Sidecar disk loads and browser fallback loads migrate valid v2 documents to v3 without regenerating IDs or changing existing bytes. Repository saves reject lifecycle-history rewrites and return path-specific validation errors.
...
- **Automated results:** 32 focused sidecar repository/route/session tests and 44 focused app storage/hook/render/IPC tests passed. Recursive TypeScript checks, lint, and formatting checks passed.

**Hard stop:** Satisfied — the canonical `fixtures/project-notes-v3.json` contract and v2-to-v3 migration/restart evidence are checked in; no Phase 17 UI or lifecycle automation was added.
...
- **Native evidence:** `.gg/capture-native.ps1` captured the same synthetic Notes fixture in a Tauri WebView at 1200×820 and 420×760 (`native-notes-typical.png`, `native-notes-narrow.png`), confirming adaptive WebView sizing and wrapped narrow actions.
- **Preferences and review:** Emulated reduced motion matched with `0s` tab transitions. Forced colors preserved a selected border and solid focus outline. Pointer-selected tabs did not match `:focus-visible`; keyboard ArrowRight focused and selected Roadmap; Escape returned focus to the Notes trigger. The rendered critique revised narrow action-row width and finished at 23/24 in `.gg/evidence/phase-17/review.md`.
- **Honest limits:** No project axe/Accessibility Insights command or field Core Web Vitals telemetry is configured. Semantic tests, browser inspection, and local Tauri evidence are recorded without claiming a complete accessibility audit or field-performance measurement.

**Hard stop:** Satisfied — desktop/narrow browser and native screenshots, keyboard/focus, 200%/320 px reflow, long/localized content, reduced motion, forced colors, every existing Notes regression, app gates, and the critique/revision cycle passed. Phase 18 is next; Phase 17 adds no Roadmap controls.
...
- Add ordered phase list/detail CRUD, status overrides, cancellation, archival, and position-preserving restoration.
- Preserve strict v3 schema compatibility, sidecar persistence, conflict replay, and accessible desktop/narrow interactions.
...
- **Implementation commit:** `6e9569b3` records the Phase 18 schema compatibility, ordered phase management, persistence replay, UI, and focused tests.
...
**Status:** Complete — implementation and isolated automated UI acceptance pass; visible packaged Windows toast delivery is deferred manual release QA.
...
- **Occurrence authority:** Notes v3 keeps a stable reminder ID plus one occurrence key and repository-owned `lastDelivery`. Legacy current-v3 reminders upgrade deterministically to `occurrenceKey: id`; generic saves cannot forge or rewrite delivery evidence; the locked targeted claim rechecks phase eligibility, archive, occurrence, due time, and prior delivery before one revision.
- **Daemon and IPC:** One coordinator watches projects with logical sessions, reloads on timer wake, caps long waits, gives focused panes a 750 ms priority window, expires session-owned leases, and broadcasts only `{}` for `roadmap_reminder_due`. Reserve/claim/release travel React → typed Tauri commands → authenticated Rust localhost proxy → shared daemon. Committed claims fan out one Notes snapshot and reschedule once.
- **Delivery policy:** Focused delivery claims `in-app` without probing permission. Background delivery shares one lazy permission promise; granted claims `native` and calls a narrow Rust command with fixed **Roadmap reminder due** / **Open GG Coder to review it.** copy, while denied claims `in-app-fallback`. Native display follows durable commit; dispatch failure logs without replay. Existing sound settings select one normal platform sound (`Submarine`, `Mail`, or `message-new-instant`) or none.
- **Scheduling/UI:** Later today is two hours ahead rounded to a quarter-hour on the same local date; Tomorrow is local 09:00; custom `datetime-local` values revalidate on conflict replay. Snooze preserves the reminder ID/note and prior delivery evidence while generating a new occurrence; Dismiss and Resume cleanup preserve phase status. Notes exposes a due-action badge, time-aware rows, exact detail/recovery copy, and one queued non-modal alert.
- **Automated results:** Focused backend gate passed 157 tests; focused app gate passed 172 tests; locked Rust passed 114 tests. Full `@kenkaiiii/ggcoder` passed 2,212 tests with 15 explicit skips; full `gg-app` and its 8 workspace evidence tests passed. Checks, root/app lint and formatting, both production builds, sidecar bundling, generated audits, fixture smokes, and `git diff --check` passed. The automation-safety follow-up passed 23 affected script tests, app typecheck, Cargo check (with one pre-existing dead-code warning), three focused Rust tests, script syntax checks, Prettier/Rust formatting checks, and `git diff --check`. The 2026-07-29 isolated dev UI acceptance rerun printed `PHASE25 DEV UI PASS`, recorded exactly one authenticated focused `in-app` claim, blocked background reservation, captured alert/detail screenshots, and recorded `nativeToastAttempted: false`.
- **Rendered evidence:** `.gg/screenshots/phase25-desktop-detail.png`, `phase25-desktop-alert-final.png`, `phase25-narrow-detail.png`, and `phase25-narrow-alert-final.png` cover persistent detail, denied recovery, exact presets/custom form, and the alert at desktop/360 px. `.gg/evidence/phase-25/critique.md` records a 23/24 local-system review after moving the alert above the toaster lane. The isolated focused dev fixture captured a passing reminder alert and phase detail without requesting native notification delivery.
- **Closed-app decision:** Phase 25 is explicitly startup-only. It registers no future OS schedule while GG Coder is closed; reopening performs one overdue reservation only after authoritative Project Notes hydration.
- **Supported automated path:** Generate the isolated dev fixture with `eval "$(cd gg-app && node scripts/phase-25-dev-fixture.mjs --cdp-port 19225)"`, launch `pnpm --filter gg-app tauri dev` in that shell, then run `node gg-app/scripts/phase-25-dev-evidence.mjs` from another shell. This focused-only path uses temporary profile/data roots, seeds one overdue reminder, blocks background/native delivery, and preserves real user data. Generic and Phase 25 packaged smoke, installer execution/extraction, installed-app cleanup, process matching, shortcut/registry mutation, and notification inspection are outside normal automation.
- **Windows release candidate:** The unsigned NSIS 0.24.4 installer is `C:\ggcoder-projects\gg-framework-fork\gg-app\src-tauri\target\release\bundle\nsis\GG Coder_0.24.4_x64-setup.exe` (72,884,198 bytes), SHA-256 `ccf7f93092f3296d64e81ec5d8dfb130c54ea0fd38a2e37c0548ae1229ea726e`. It was built from current branch `custom/local-customizations` at HEAD `a5a0c3bcf624` plus the retained Phase 25 work with `pnpm --filter @kenkaiiii/ggcoder... build && pnpm --filter gg-app stage:node && pnpm --filter gg-app bundle:sidecar && pnpm --filter gg-app tauri build --bundles nsis --no-sign --ci`. The inspected tag workflow uses the same framework build, Node staging, sidecar bundling, and Tauri packaging path; release CI additionally signs updater artifacts when its secrets are present. Its stale packaged-smoke invocation and obsolete smoke-evidence uploads were removed so retired automation cannot block the release build. The installer was not executed, extracted, installed, or launched on the build host.
- **Deferred manual release QA — packaged Windows toast:** In a fresh disposable Windows Sandbox/VM only: (1) copy only that installer into the guest, run `Get-FileHash -Algorithm SHA256 '<installer>'`, and require the hash above before installing; (2) enable guest notifications, install inside the guest, launch from the installer-created **GG Coder** desktop shortcut, and choose a disposable empty project folder; (3) create an active Roadmap phase, set a custom reminder two minutes ahead, close Notes, and minimize GG Coder; (4) capture one full-guest screenshot while the native toast visibly shows **Roadmap reminder due** / **Open GG Coder to review it.** with the guest clock; and (5) record guest Windows version, timezone, timestamp, app version, installer filename, and SHA-256, confirm no duplicate toast for that occurrence, export only the evidence, and discard the guest.
- **Honest limits:** The isolated dev fixture proves focused in-app alert/detail behavior and fixture isolation only. It does not prove Windows AppUserModelID registration, packaged execution, or visible native toast delivery. The visible packaged Windows toast remains deferred manual release QA. A real two-window same-project manual audit, macOS notification/sound, native screen-reader speech, manual 320 px keyboard/pointer/RTL/forced-colors/200% interaction, and field performance remain unverified.

**Hard stop:** Satisfied — implementation gates and the isolated focused UI acceptance fixture pass. Visible packaged Windows toast delivery remains an explicitly separate deferred manual release QA check and is not inferred from dev evidence.

## Phase 26 — Run the Notes Roadmap release gate

**Status:** In progress — local and isolated Windows gates pass; exact-reviewed-diff CI and isolated macOS desktop evidence are missing.
...
**Provisional requirement matrix — reviewed by Autopilot Ken on 2026-07-29**

Reviewed state: HEAD `a5a0c3bcf624e7976d42d8589d26fe1a5629e419` plus the 46-entry `git status --short` working tree (34 tracked and 12 untracked). This is not a commit identity, so bare-HEAD CI does not cover the reviewed state.

| Gate                                                                                                                                                                                                | State                        | Fresh evidence                                                                                                                                                                                                                                                      | Owning files                                                                                                                                                                                                                                             |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Migration, malformed primary/backup recovery, old Notes preservation, restart durability                                                                                                            | Green locally                | Focused matrices: backend 249/249 and app 255/255. Full suites: ggcoder 2,213 passed / 15 explicit skips; app 601 plus 8 workspace evidence tests.                                                                                                                  | `packages/ggcoder/src/project-notes-repository.test.ts`; `gg-app/src/notes-storage.test.ts`; `gg-app/src/useProjectNotes.test.tsx`                                                                                                                       |
| Saved prompt/reference through Start, approval, complete checkpoint, passed verification, reviewer acceptance, one non-archived Done, restart/Resume, chronological history, and phase-only package | Green locally                | New production-orchestration case passed inside `app-sidecar-phase.test.ts` (38/38 file tests); focused context/UI suites also passed.                                                                                                                              | `packages/ggcoder/src/app-sidecar-phase.test.ts`; `packages/ggcoder/src/phase-context.test.ts`; `gg-app/src/ProjectNotes.test.tsx`; `gg-app/src/AgentPane.test.tsx`                                                                                      |
| Duplicate/failed launch, stale write/session, lease races, manual overrides, rejected review, denied notification, reminder deduplication                                                           | Green locally                | Focused backend/app matrices passed 504 tests total; locked Rust passed 117/117.                                                                                                                                                                                    | `packages/ggcoder/src/app-sidecar-phase.test.ts`; `packages/ggcoder/src/project-notes-repository.test.ts`; `packages/ggcoder/src/app-sidecar-reminders.test.ts`; `gg-app/src/roadmap-reminders.test.ts`; `gg-app/scripts/phase-25-native-smoke.test.mjs` |
| Isolated focused UI, one in-app claim, no background reservation, `nativeToastAttempted: false`                                                                                                     | Green on Windows             | `PHASE25 DEV UI PASS`; exactly one authenticated focused `in-app` / `not-required` claim, zero background reserve attempts, alert/detail screenshots inspected, `nativeToastAttempted: false`.                                                                      | `gg-app/scripts/phase-25-dev-fixture.mjs`; `gg-app/scripts/phase-25-dev-evidence.mjs`; `gg-app/scripts/phase-25-sidecar-fixture.mjs`                                                                                                                     |
| Full package/Rust/generated/build gates plus exact-current-reviewed-commit six-job CI                                                                                                               | Blocked on CI identity       | Local gates all pass. Run [`30421549329`](https://github.com/creativeprofit22/gg-framework/actions/runs/30421549329) has six green Windows/macOS/Linux jobs for bare HEAD `a5a0c3bc`, but predates the 46-entry reviewed diff and is not counted.                   | Root/package scripts; `gg-app/src-tauri`; `.github/workflows/ci.yml`                                                                                                                                                                                     |
| Current-checkout Windows and macOS isolated `tauri dev` smoke                                                                                                                                       | Windows green; macOS missing | Windows 10 Pro `10.0.19045` x64: current checkout at HEAD plus retained diff; fixture-listening→durable claim elapsed 23.845 s; fixture-owned process tree stopped; evidence below. No isolated macOS runner was available, and CI compilation was not substituted. | Phase 25 dev fixture/evidence scripts; macOS desktop smoke still required                                                                                                                                                                                |

**Fresh command evidence**

- **Focused Track B:** The two approved `vitest run` matrices passed backend 249/249 across 10 files and app 255/255 across 10 files. Existing React `act(...)` and jsdom canvas diagnostics remained non-failing.
- **Full local:** `pnpm --filter @kenkaiiii/ggcoder test` passed 2,213 with 15 explicit skips; `pnpm --filter gg-app test` passed 601 plus 8 workspace evidence tests; `cargo test --locked` from `gg-app/src-tauri` passed 117. The approved `pnpm --dir gg-app/src-tauri exec cargo test --locked` wrapper first reported that `src-tauri` is not a pnpm package, so the identical locked Cargo gate ran from its actual directory.
- **Repository gates:** `pnpm check`, `pnpm lint`, `pnpm format:check`, `pnpm build`, `pnpm --filter gg-app stage:node`, `pnpm --filter gg-app bundle:sidecar`, `node gg-app/scripts/smoke-sidecar.mjs`, all seven generated-output audit commands, and `git diff --check` passed. Generated outputs were inspected and remained ignored/untracked.
- **Windows desktop:** The fixture redirected `HOME`, `USERPROFILE`, `APPDATA`, `LOCALAPPDATA`, `TEMP`, `TMP`, and `WEBVIEW2_USER_DATA_FOLDER` inside `C:\Users\SPARTA~1\AppData\Local\Temp\gg-app-phase25-dev-TMsDp3`. It launched only `pnpm --filter gg-app tauri dev`, then ran `node gg-app/scripts/phase-25-dev-evidence.mjs`. Evidence: `screenshots/evidence.json`, `screenshots/reminder-alert.png`, `screenshots/reminder-detail.png`, and `audit/fixture-audit.jsonl` under that root. The fixture-owned process tree was stopped after capture; its forced dev-server stop reported exit code 1 as expected.
- **CI/macOS:** Exact-reviewed-diff six-job CI cannot exist until the retained diff has a commit identity. This Windows host has no isolated macOS runner, so the required Notes open + bound-phase inspect/Resume smoke remains unrun.
- **Native limitation:** Focused dev evidence does not execute packaging, validate AppUserModelID registration, or attempt native toast delivery. `nativeToastAttempted: false` is a required safety result, not native-toast proof; visible packaged Windows toast remains deferred manual release QA.

**Hard stop:** Open — Autopilot Ken has not signed off Phase 26 because exact-reviewed-diff six-job CI and a real isolated macOS `tauri dev` Notes inspect/Resume result are missing. Release remains a separate explicit task.
