...
import { useAgentEvents, HOOK_PRESENTATION } from "./useAgentEvents";
...
export function AgentPane({
  ...
  paneId = suppliedClient?.paneId ?? "primary",
  focused = true,
  windowFocused: workspaceWindowFocused = true,
  initialTarget,
  ...
  useEffect(() => {
    ...
    const shouldRestore =
      workspaceOwnsSessionLifecycle || reclaimNativeSession || existingGeneration != null;
    const operation = shouldRestore ? client.restore(target) : client.create(target);
    void operation
      .then((next) => {
        if (!workspaceOwnsSessionLifecycle) ownedGeneration = next;
    ...
    return () => {
      ...
      if (!workspaceOwnsSessionLifecycle && ownedGeneration !== null)
        void client.dispose(ownedGeneration).catch(() => {});
    };
  }, [client, reclaimNativeSession, target, workspaceOwnsSessionLifecycle]);
  ...
  const hydrate = useCallback(async (): Promise<void> => {
    ...
    const current = (): boolean => mountedRef.current && hydrateEpochRef.current === epoch;
    ...
    try {
      ...
      if (!current()) return;
      if (st) {
        setWorkspaceMode(st.mode ?? target?.mode ?? "code");
        setState(st);
  ...
  // Re-point this pane at a freshly chosen project: clear the old transcript
  // and force a re-hydrate against the replacement native session.
  const onProjectChosen = useCallback((): void => {
    hydrateEpochRef.current += 1;
    void client
      .status()
      .then((status) => {
        ...
        onGenerationChangeRef.current?.(status.generation);
      })
      .catch(() => {});
    stickToBottomRef.current = true;
    setItems([]);
  ...
  useEffect(() => {
    if (kind !== "primary" || workspaceOwnsSessionLifecycle) {
  ...
  }, [kind, onProjectChosen, workspaceOwnsSessionLifecycle]);
  ...
  const handleDeleteTask = useCallback(
    (id: string) => {
      void client.deleteTask(id).then(setProjectTasks);
    },
    [client],
  );
  ...
  function onSelectKenModel(modelId: string | null): void {
    ...
    void client.switchKenModel(modelId).then((res) => {
      if (res) {
        setState((s) =>
          s
            ? {
  ...
  function onSelectModel(modelId: string): void {
    ...
    void client.switchModel(modelId).then((res) => {
      if (res) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    const t = setTimeout(() => {
      void client.searchFiles(mention.query).then((files) => {
        if (!cancelled) {
  ...
  async function runEnhance(): Promise<void> {
    ...
    if (reduced) {
      try {
        applyEnhanceResult(await client.enhancePrompt(draft));
      } catch {
    ...
    try {
      const r = await client.enhancePrompt(draft);
  ...
  function submit(): void {
    ...
    if (kenMatch) {
      ...
      setMentionedPaths([]);
      setEnhancement(null);
      void client.sendKenPrompt(question);
      return;
    }
