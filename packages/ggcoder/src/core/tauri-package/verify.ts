...
export async function verifyTauriPackage(repositoryRoot: string): Promise<VerificationResult> {
  ...
  try {
    const content = await readFile(path.join(repositoryRoot, ...GENERATED_PATHS[4].split("/")), "utf8");
    const value = JSON.parse(content) as unknown;
    parsed = validateConfig(value);
    if (canonicalJson(parsed) !== content) errors.push(`${GENERATED_PATHS[4]}: configuration is not canonical JSON`);
  } catch (error) {
  ...
  if (parsed) {
    ...
    try {
      ...
      const target = discovery.targets.find((candidate) => candidate.target_id === parsed!.target.target_id);
      if (!target) errors.push(`Configured target is no longer discoverable: ${parsed.target.target_id}`);
      else if (stableJson(target) !== stableJson(parsed.target)) errors.push("Configured target differs from deterministic discovery");
...
function validateConfig(value: unknown): GeneratedTauriConfig {
  ...
  if (value.schema_version !== GENERATED_CONFIG_SCHEMA_VERSION) throw new Error("unsupported configuration schema");
  ...
  if (!Array.isArray(value.sources) || !isObject(value.target)) throw new Error("invalid source or target data");
...
function validateCalibration(value: unknown, evidenceSha256: string): asserts value is CalibrationBounds {
  ...
  if (value.evidence_sha256 !== evidenceSha256) throw new Error("calibration evidence digest is stale");
  for (const field of [
    "absolute_growth_bytes",
    "baseline_total_bytes",
    "maximum_file_bytes",
  ...
  if (!Array.isArray(value.required_paths) || value.required_paths.some((item) => typeof item !== "string")) {
