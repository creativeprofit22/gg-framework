...
const mocks = vi.hoisted(() => ({ isTauri: vi.fn(() => true), invoke: vi.fn(async (..._args: unknown[]) => {}), toast: vi.fn() }));
vi.mock("@tauri-apps/api/core", async importOriginal => ({ ...await importOriginal<typeof TauriCore>(), isTauri: mocks.isTauri }));
...
afterEach(() => { vi.clearAllMocks(); vi.restoreAllMocks(); localStorage.clear(); });
...
describe("native appearance", () => {
  it("initializes each window from saved settings and ignores reading-only changes", async () => {
    ...
    const first = startNativeAppearance(); await flush();
    ...
    appearance.update({ size: "16" }); await flush();
    expect(mocks.invoke).toHaveBeenCalledTimes(2);
    const second = startNativeAppearance(); await flush();
    expect(mocks.invoke).toHaveBeenCalledTimes(4);
    first(); second(); stopOwner();
  });
  it.each(["main", "project-regression", "whatsnew"])("serializes both colors for the actual %s window", async currentLabel => {
    label = currentLabel;
    const stopOwner = appearance.start();
  ...
  it("reports background IPC rejection and recovers on the next theme", async () => {
    ...
    appearance.update({ theme: "light" });
    await flush();
    expect(mocks.invoke).toHaveBeenLastCalledWith("plugin:window|set_background_color", { label: "main", value: "#fcfbfd" }, undefined);
    stop(); stopOwner();
  });
  it("serializes and coalesces rapid toggles so the latest native update wins", async () => {
    let resolve!: () => void;
    const target = { setTheme: vi.fn<(_: "dark" | "light") => Promise<void>>().mockImplementationOnce(() => new Promise<void>(r => { resolve = r; })).mockResolvedValue(undefined), setBackgroundColor: vi.fn(async () => {}) };
    const sync = createNativeAppearanceSync(target, vi.fn());
    sync.update("dark"); sync.update("light"); sync.update("dark"); sync.update("light");
  ...
  it("reports failure without preventing subsequent changes", async () => {
    const report = vi.fn();
    const target = { setTheme: vi.fn().mockRejectedValueOnce(new Error("permission denied")).mockResolvedValue(undefined), setBackgroundColor: vi.fn(async () => {}) };
    const sync = createNativeAppearanceSync(target, report);
    sync.update("light"); await flush(); expect(report).toHaveBeenCalledOnce();
    sync.update("dark"); await flush(); expect(target.setBackgroundColor).toHaveBeenLastCalledWith("#0f1115");
    sync.stop();
  });
  it("does not issue background work after teardown", async () => {
    let resolve!: () => void;
    const target = { setTheme: vi.fn(() => new Promise<void>(r => { resolve = r; })), setBackgroundColor: vi.fn(async () => {}) };
    const sync = createNativeAppearanceSync(target, vi.fn());
    sync.update("light"); sync.stop(); resolve(); await flush(); sync.update("dark");
    expect(target.setTheme).toHaveBeenCalledOnce(); expect(target.setBackgroundColor).not.toHaveBeenCalled();
  });
