...
Apply this rule before creating support files or auditing categories: a brand-new empty project gets exactly "Empty project — nothing to audit. To bootstrap, tell me the stack you want and I'll scaffold from scratch." and then stop. A non-empty project whose stack is unknown still receives valid no-target generated cleanup support.
...
Inspect only root manifests, directly declared workspace manifests, canonical build configs referenced by them, root/workspace ignore files, and read-only Git metadata. Use \`ls\`, \`find\`, and \`read\` for files. Use \`bash\` only for non-mutating Git probes, running the generated Node program/tests, and final verification. Do not crawl or grep source trees, import JavaScript/TypeScript config, execute project build config, or guess from directory names.

Before writing, state a private candidate registry containing each target ID, literal path, generation evidence, and evidence file. Follow these rules:

1. Extract explicit output declarations first: compiler/bundler output directories, build-command output flags, Cargo target configuration, Tauri generated-schema locations, package entry-point directories corroborated by build scripts, and equivalent canonical declarations for the detected stack.
2. Treat conventional outputs such as Vite \`dist\`, Next \`.next\`, Cargo \`target\`, or Tauri \`gen/schemas\` as candidates only when matching manifest/build/config evidence establishes the convention. Language or directory presence alone is insufficient.
3. Expand workspace declarations only to direct, existing package roots and persist literal paths such as \`packages/a/dist\`; never persist \`packages/*/dist\` or any glob.
4. Query ignore provenance with argv-safe \`git check-ignore --no-index -v -z --stdin\` so force-added tracked descendants retain their ignore rule, and tracked descendants with \`git ls-files -z -- <literal-path>\`. Invoke Git without a shell pipeline. Ignore rules may corroborate, but never create, a candidate.
5. Normalize display paths to \`/\`, sort lexically, merge duplicate paths/evidence so each literal path appears in exactly one stable target, and collapse redundant parent/child candidates to the narrowest evidence-backed representation.
6. Derive stable target IDs from evidence-backed roles (for example \`web\`, \`cargo\`, \`tauri-schemas\`, or package-qualified targets). An unknown or README-only project has an empty target list and a valid no-target cleanup command; never guess \`dist\`, \`build\`, cache, or dependency directories.
7. Report dynamic or ambiguous build config as unsupported rather than importing, executing, or guessing it. Inspection output must name the exact evidence files for every target.

### Generated audit program contract

The generated \`scripts/audit-generated.mjs\` must export pure discovery, render, and audit functions for its test and expose only these read-only CLI forms:

- \`node scripts/audit-generated.mjs --inspect\` emits stable JSON with detected targets, literal paths, evidence, evidence files, and the exact cleanup Markdown.
- \`node scripts/audit-generated.mjs <target>\` audits exactly one detected target and reports its evidence plus present/missing, tracked/untracked, ignored/not-ignored, and matching ignore provenance for every literal path.
- Help is allowed. Explicitly reject \`--apply\`, \`--delete\`, \`--clean\`, \`--force\`, multiple targets, and unknown targets.

Keep production physically read-only:

1. Production may use filesystem reads such as \`existsSync\`, \`lstat\`, \`readFile\`, and \`readdir\`, plus shell-free \`execFile\`. It must not import or call filesystem mutators such as \`writeFile\`, \`appendFile\`, \`copyFile\`, \`cp\`, \`mkdir\`, \`rm\`, \`rmdir\`, \`unlink\`, \`rename\`, \`chmod\`, or synchronous equivalents.
2. Require every selected path to be non-empty, literal, repository-relative, free of glob/traversal segments, and resolved beneath the repository root.
3. Protect \`.git\`, all \`.gg\` runtime/evidence directories, root \`node_modules\`, credential/secret paths encountered during inspection, and bundle inputs such as \`src-tauri/binaries\`. Reject a candidate that equals, contains, or is an ancestor of a protected path.
4. Before Git probes, \`lstat\` every existing segment from the repository root. Recursively inspect selected directories in lexical order and refuse symbolic links, Windows junctions, nested \`.git\` directories, nested \`.git\` worktree files, and unsupported filesystem objects with a path-specific \`Refusing to audit ...\` error.
5. Run Git through \`execFile\` with \`shell: false\`. Only check-ignore exit 1 means no match; surface every other Git/spawn failure.
6. The program only reports. It has no deletion function, and the generated cleanup prompt must forbid follow-up deletion or mutation.

### Deterministic generated fixtures

Generate \`scripts/audit-generated.test.mjs\` with \`node:test\`, temporary Git repositories, and Node standard-library modules only. Test exported functions and spawned CLI on Windows and POSIX, using native filesystem paths internally and \`/\` only for display. Cover:

- Node fixtures: Vite and TypeScript manifest/config evidence, ignored and missing outputs, a force-added generated descendant, exact ignore provenance, stable target IDs, and no inferred output from an unrelated Node manifest.
- Rust/Tauri fixtures: \`Cargo.toml\`, optional \`.cargo/config.toml\` target-dir, \`src-tauri/Cargo.toml\`, Tauri config, matching ignores, only evidence-backed Cargo/Tauri targets, literal paths, nested \`.git\` directory/worktree-file refusals, and a real directory symlink (a junction on Windows).
- Monorepo fixtures: at least two directly declared packages with different build outputs; assert sorted literal expansion, no globs, exclusion of undeclared/unbuildable packages, and parent/child deduplication.
- Unknown fixture: a README-only Git project with empty targets, no conventional guesses, deterministic cleanup Markdown, and a clean no-target result.
- Rerun/idempotency: render/apply the generated support twice and assert byte-identical files, modes, ordering, and Git status on the second pass.
...
8. Immediately before reporting, run the complete fixture test, compare two \`--inspect\` results and the rendered cleanup Markdown byte-for-byte, and run the representative/no-target audit with the in-memory status check. If any later change touches a support file, repeat this terminal gate; never report completion after a failed terminal run.
9. Continue with the six-category report below. Add a concise \`Generated cleanup support\` section naming the three paths, detected targets or \`none\`, fixture-test result, idempotency result, audit result, and preserved conflicts.
...
## How to investigate the six-category audit

- Reuse the bounded root/config inspection already performed for generated support.
...
A single Markdown report, organized by category and including the \`Generated cleanup support\` section. Within each audit category, mark each item as one of:
