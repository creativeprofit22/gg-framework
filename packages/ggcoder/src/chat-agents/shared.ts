...
export function buildChatAgentSystemPrompt(
  ...
  const canMutateDurableContext = agentId !== "research";
  const handoffInstructions =
    handoffEnabled && agentId !== "research"
      ? `\n\nAgent handoff:\nYou can call delegate_to_agent to hand the entire conversation to a better-suited agent. Hand off to Therapist when the conversation primarily needs emotional support, reflection, coping, relationships, or wellbeing. Hand off to Research when it primarily needs evidence gathering, source verification, comparisons, current information, or deep analysis. Hand off to Brainstorm when the conversation needs ideation, problem framing, or broad discussion that no longer needs a specialist. A handoff changes the active agent for this conversation; it is not a one-off subtask. Use it when the conversation's primary need clearly shifts, not for a brief side question or work you can handle naturally.`
      : "";
  const durableContextInstructions = canMutateDurableContext
    ? `${MEMORY_CURATION_INSTRUCTIONS}${JIWA_CURATION_INSTRUCTIONS}`
    : "";
  ...
  return `${rolePrompt}${durableContextInstructions}${handoffInstructions}\n\n${runtimeContext}`;
}
