...
import { workspaceLayoutLeafIds, type WorkspaceLayout, type WorkspaceLayoutNode } from "./workspace-layout";

export interface WorkspaceSize { width: number; height: number }
export interface PaneRect { paneId: string; left: number; top: number; width: number; height: number }
export type SwapRow = { available: true; paneIds: string[]; middleId: string } |
...
export function workspacePaneRects(root: WorkspaceLayoutNode, size: WorkspaceSize): PaneRect[] {
  const resolve = (length: LayoutLength, extent: number) => extent * length.percent / 100 + length.pixels;
  return [...workspaceGeometry(root).paneGeometry.values()].map(({ paneId, rect }) => ({
    paneId, left: resolve(rect.left, size.width), top: resolve(rect.top, size.height),
    width: resolve(rect.width, size.width), height: resolve(rect.height, size.height),
  }));
}
...
export function findSwapRow(rects: readonly PaneRect[], paneId: string): SwapRow {
  ...
  if (rects.some((r) => ![r.left, r.top, r.width, r.height].every(Number.isFinite) || r.width <= 0 || r.height <= 0)) {
    return unavailable("Wait for the workspace layout to become visible.");
  }
  const bottom = (r: PaneRect) => r.top + r.height;
  const aligned = rects.filter((r) => Math.abs(r.top - origin.top) <= tolerance &&
    Math.abs(bottom(r) - bottom(origin)) <= tolerance).sort((a, b) => a.left - b.left);
  const index = aligned.indexOf(origin);
  const adjacent = (a: PaneRect, b: PaneRect) => Math.abs(b.left - a.left - a.width - DIVIDER_SIZE_PX) <= tolerance;
  let first = index;
  let last = index;
  ...
  if (Math.max(...row.map((r) => r.top)) - Math.min(...row.map((r) => r.top)) > tolerance ||
  ...
  if (rects.some((r) => !row.includes(r) && r.left < right && r.left + r.width > left &&
      r.top < bottom(origin) - tolerance && bottom(r) > origin.top + tolerance)) {
    return unavailable("Swap needs an unambiguous aligned row.");
  }
  const midpoint = (left + right) / 2;
  let middle = row[0];
  for (const candidate of row.slice(1)) {
    if (Math.abs(candidate.left + candidate.width / 2 - midpoint) <
        Math.abs(middle.left + middle.width / 2 - midpoint) - 1e-7) middle = candidate;
  }
  return { available: true, paneIds: row.map((r) => r.paneId), middleId: middle.paneId };
}

export function swapWorkspacePanes(
  layout: WorkspaceLayout, sideId: string, expectedMiddleId: string, size: WorkspaceSize,
): WorkspaceLayout {
  if (sideId === expectedMiddleId || !Object.prototype.hasOwnProperty.call(layout.panes, sideId) ||
      !Object.prototype.hasOwnProperty.call(layout.panes, expectedMiddleId)) return layout;
  const ids = workspaceLayoutLeafIds(layout.root);
  if (ids.filter((id) => id === sideId).length !== 1 || ids.filter((id) => id === expectedMiddleId).length !== 1) return layout;
