...
export function NotesPhaseDeletionDialog({ controller }: { controller: PhaseDeletionController }) {
  ...
  return (
    <Modal
      title={`${deleting ? "Delete" : "Recover"} “${target.phase.title}”?`}
      className="notes-phase-deletion-dialog"
      ...
      <div className="notes-phase-deletion-body">
        <p>
          {deleting
            ...
            : "Recovery returns the phase to its original active or archived location. Completed phases keep Done as history. Other phases return as not-started and need fresh execution and plan approval. Past sessions and reminders are not resumed."}
        </p>
        <p>Deleted phases are kept indefinitely. There is no permanent purge.</p>
        {preparing && <p role="status">Saving pending edits…</p>}
        {pending && (
          <p role="status">
            {deleting ? "Deleting…" : "Recovering…"} The submitted write cannot be cancelled here.
          </p>
        )}
        ...
        <div className="notes-phase-deletion-actions">
          ...
          {!request && !preparing && (
            <button
              className="notes-roadmap-new"
              type="button"
              onClick={() => void controller.begin(target.phase, target.action)}
            >
              Review latest phase
            </button>
          )}
