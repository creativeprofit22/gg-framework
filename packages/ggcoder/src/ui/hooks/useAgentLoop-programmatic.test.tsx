...
} from "../../core/programmatic/assessment.js";
...
it.each(["unavailable", "denied", "cancelled", "failed"] as const)(
  "retains material %s scan and history limits without emitting the projection",
  (status) => {
    const outcome: ProgrammaticAssessmentOutcome = {
      assessment: {
        version: 1,
        mode: "configured",
        status: "unavailable",
        summary: "Assessment is unavailable.",
        deterministic: { status, reason: `Saved checks ${status}. No retry was attempted.` },
        history: {
          status: "acknowledgement-unknown",
          assessmentId: "private-history-id",
          reason: "The save acknowledgement was lost. Check history before saving again.",
        },
        coverage: [
          {
            scope: "project",
            status: "unreadable",
            summary: "Some files could not be read safely.",
          },
        ],
        observations: [
          { basis: "observed", message: "An observation", evidenceSources: ["private-receipt-id"] },
        ],
...
it("bounds oversized accepted display text without hiding scan/history failure or claiming full coverage", () => {
  const outcome: ProgrammaticAssessmentOutcome = {
    assessment: {
      ...
      mode: "configured",
      status: "incomplete",
      summary: "Assessment did not finish.",
      deterministic: { status: "failed", reason: "Saved checks failed. No retry was attempted." },
      history: {
        status: "unsaved",
        assessmentId: "private-history-id",
        reason: "Saving was denied.",
      },
      coverage: [],
      observations: [],
      limitations: Array.from({ length: 50 }, (_, index) => `${index}: ${"limit ".repeat(660)}`),
    },
...
it.each([
  ...
  "projects terminal history independently after a real assessment: %s",
  async (scenario) => {
    ...
    function Harness() {
      loop = useAgentLoop(
        messages,
        { provider: "openai", model: "gpt-5", tools, planModeRef, maxTokens: 100 },
        { onTurnText },
      );
      return null;
    }
    ...
    try {
      ...
      const proposal = await buildProgrammaticProfileProposal(cwd, {
        offerHistory: !scenario.startsWith("legacy-"),
      });
      expect(
        (
            ...
            if (scenario === "acknowledgement-unknown") {
              ...
              args[5] = {
                ...
                assertCurrent: () => {
                  ...
                  if (scenario === "tool-at-commit")
                    tools.splice(tools.indexOf(scan), 1, { ...scan });
                  assertCurrent!();
                },
              };
            }
            ...
            (async function* () {
              ...
              expect(
                params.tools?.some((tool) =>
                  /history.*(save|write)|recommendation.*(save|write)/.test(tool.name),
                ),
              ).toBe(false);
                    ...
                    recommendations: [
                      {
                        ...
                        workflow: {
                          trigger: "A manifest change needs review",
                          representativeCase: "Review this manifest",
                          inputs: ["package.json"],
                          currentProcess: ["Read manifest"],
                          output: "Bounded review",
                          successCheck: "Trace each observation to source",
                          affectedSubproject: { scope: "repository-wide" },
                          mutationBoundary: "Read-only; changes need separate approval",
                          repeatability: {
                            basis: "inferred",
                            explanation: "Source changes may require review again",
                          },
                        },
                        alternatives: [],
              ...
              if (scenario !== "setup")
                scannerBytes = await fs.readFile(path.join(cwd, PROGRAMMATIC_STATE_PATH));
              if (scenario === "profile-replaced")
                await fs.writeFile(profilePath, Buffer.concat([profileBefore, Buffer.from("\n")]));
              if (scenario === "revoked") {
                const profile = JSON.parse(await fs.readFile(profilePath, "utf8"));
                profile.historyPolicy.enabled = false;
                await fs.writeFile(profilePath, JSON.stringify(profile));
              ...
              const content =
                scenario === "report-rendered"
                  ? String(
                      params.messages
                        .flatMap((message) => (message.role === "tool" ? message.content : []))
                        .find((result) => result.toolCallId === "history-advice")!.content,
                    )
                  : "Finished fixture.";
      ...
      expect(
        await submitPromptCommand({
          cwd,
          trimmed: scenario === "setup" ? "/setup-programmatic" : "/programmatic",
          inputImages: [],
          currentModel: "gpt-5",
          ...
          runAgent: loop.run,
          isBusy: loop.isBusy,
          setLiveItems: guidance,
          getId: () => "history-fixture",
          reloadCustomCommands: vi.fn(),
        }),
      ).toBe(true);
      const notices = messages.current.filter(
        (message) =>
      ...
      expect(presentation).not.toMatch(
        /"(?:discovery|evidenceSources|candidateId|assessmentId|version)":/,
      );
      ...
      if (scenario === "acknowledgement-unknown") {
        ...
        expect(presentation).toContain(
          assessment.history && "reason" in assessment.history
            ? assessment.history.reason
            : "missing reason",
        );
      }
      ...
      if (assessment.status === "completed") {
        ...
        if (scenario === "report-rendered")
          expect(presentation).not.toContain("1. Review manifest");
        else expect(presentation).toContain("1. Review manifest");
      }
      const expectedHistoryStatus =
        scenario === "setup"
          ? "setup-not-saved"
          : scenario.startsWith("legacy-") || scenario === "disabled"
            ? "disabled"
            : scenario === "acknowledgement-unknown"
              ? "acknowledgement-unknown"
              : ["revoked", "profile-replaced", "cancelled", "reset", "save-failed"].includes(
                    scenario,
                  ) || scenario.endsWith("-at-commit")
                ? "unsaved"
                : "saved";
      expect(assessment.history?.status).toBe(expectedHistoryStatus);
      expect(commitsAttempted).toBe(
        scenario.endsWith("-at-commit") || scenario === "acknowledgement-unknown" ? 1 : 0,
      );
      if (scannerBytes)
        expect(await fs.readFile(path.join(cwd, PROGRAMMATIC_STATE_PATH))).toEqual(scannerBytes);
      expect(assessment.status).toBe(
        ["cancelled", "reset"].includes(scenario)
          ? "cancelled"
          : scenario === "incomplete"
            ? "incomplete"
            : "completed",
      ...
      if (scenario === "setup")
        expect(profileCalls.mock.calls[0]![0]).toEqual({ action: "inspect" });
      ...
      if (
        ...
        if (
          assessment.history?.status !== "saved" &&
          assessment.history?.status !== "acknowledgement-unknown"
        )
          throw new Error("Expected saved or uncertain history acknowledgement");
      ...
      if (scenario !== "revoked" && scenario !== "profile-replaced")
        expect(await fs.readFile(profilePath)).toEqual(profileBefore);
    } finally {
      mounted.unmount();
      restore();
...
it.each(["result", "error", "abort", "reset", "disposal", "max-turn", "plan"])(
  ...
  async (ending) => {
    ...
    try {
            ...
            (async function* () {
              ...
              if (turn === 1) {
                ...
                calls = [
                  ...
                  ...blockedNames.map((name): ToolCall => ({
                    type: "tool_call",
                    id: `denied-${name}`,
                    name,
                    args: {},
                      ...
                      recommendations: [
                        {
                          ...
                          workflow: {
                            trigger: "A manifest setting needs comparison",
                            representativeCase: "Compare the fixture manifest setting",
                            inputs: ["package.json"],
                            currentProcess: ["Read manifest", "Compare the setting manually"],
                            output: "Bounded comparison",
                            successCheck: "Trace each comparison to inspected source",
                            affectedSubproject: { scope: "repository-wide" },
                            mutationBoundary: "Read-only; edits need separate approval",
                            repeatability: {
                              basis: "inferred",
                              explanation:
                                "Manifest changes may need repeated comparison; no measured frequency",
                            },
                          },
                          alternatives: [
                            {
                              kind: "manual",
                              reasonNotSelected:
                                "Repeated comparison benefits from the existing review procedure",
                            },
                          ],
                          choice: {
                            kind: "reuse-command",
      ...
      const notice = messages.current.find(
        (message) =>
          message.role === "assistant" &&
          typeof message.content === "string" &&
          message.content.startsWith("## Needs assessment\n\n"),
      ...
      expect(String(notice!.content)).not.toMatch(
        /"(?:discovery|evidenceSources|candidateId|assessmentId|version)":/,
      );
      expect(String(notice!.content)).toContain(
        ending === "plan"
      ...
      expect(assessment.status).toBe(
        ["abort", "reset", "disposal"].includes(ending)
          ? "cancelled"
          : ["result", "plan"].includes(ending)
            ? "completed"
            : "incomplete",
      );
      expect(assessment.deterministic).toMatchObject({
        status: ending === "plan" ? "failed" : "succeeded",
      });
      ...
      if (ending !== "reset") {
        ...
        const prompt = messages.current.find(
          (message) =>
            message.role === "user" &&
            typeof message.content === "string" &&
