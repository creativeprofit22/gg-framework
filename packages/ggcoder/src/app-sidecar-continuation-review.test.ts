...
import { buildKenAutopilotSessionContext, buildKenAutopilotPlanSessionContext, runKenAutopilotSessionReview } from "./core/ken-context.js";
...
import { createContinuationPromptAdapter, createStrandedQueueDrain, type UserTurnDeps } from "./app-sidecar-user-turn.js";
...
function fixture(kind = "eligible") {
  ...
  const state = (): ReturnType<AgentSession["getState"]> => ({
    ...
    openAICodexContextProfile: "stable", openAICodexContextProfileEligibility: { canChange: true }, openAICodexFast: false,
  });
  const session = {
    getState: state,
    ...
    getContinuationSourceRevision: () => ({ ...identity, fingerprint: sourceFingerprint(messages) }),
  ...
  const drainQueue = vi.fn(async () => { events.push("queue"); await queue.promise; });
  ...
  const runAgent = vi.fn(async (_text: string, run: () => Promise<void>) => {
    ...
    try { await run(); } catch { /* surfaced by runAgent, not thrown to startup */ }
  });
  const userTurn: UserTurnDeps = {
    runAgent, getMessages: () => messages,
    clearCancelled: () => { cancelled = false; },
    gateState: () => ({ enabled: kind !== "disabled", cancelled, planMode: kind === "plan-mode", planPending: kind === "pending-plan" || kind === "failed-plan" }),
    decision: vi.fn(), drainQueue: coordinatedDrain,
  ...
  const prompt = vi.fn(async (text: string, onAccepted: () => Promise<void>) => {
    ...
    else if (kind !== "registry") messages.push({ role: "assistant", content: [
      { type: "text", text: "Implemented and checked." },
      { type: "tool_call", id: "tool-1", name: kind === "mechanical" ? "read" : kind === "background" || kind === "git" ? "bash" : "edit",
        args: kind === "background" ? { command: "pnpm dev", run_in_background: true }
          : kind === "git" ? { command: "git commit -m fixture" } : { file_path: "fixture.ts" } },
    ] });
    if (kind === "cancelled") cancelled = true;
    if (kind === "build-failure" || kind === "failed-plan") throw new Error("provider failed AFTER output");
  });
  const adapter = vi.fn(createContinuationPromptAdapter({ prompt, userTurn,
    workflowCommand: async (text) => isWorkflowCommandText(text, [{ name: "compare", prompt: "compare template" }]),
  }));
  const service = new AppSidecarContinuationSession({
    session, mutations, runClaim, busy: () => false, reset,
    prepare: async (text) => {
    ...
    prompt: adapter, accepted: () => { events.push("accepted"); },
  });
  return { service, build, reviewer, queue, events, runClaim, mutations, review, resetReviewer, reset, prompt, drainQueue, coordinatedDrain, adapter };
}
...
describe("production continuation reviewer context persistence (B)", () => {
  it("pins historical task, approved plan and exact accepted instruction independently of review startup", async () => {
    ...
    try {
      ...
      await fs.writeFile(path.join(tmpHome, ".gg", "auth.json"), JSON.stringify({ openai: {
      ...
      realSessionAgentLoop.mockImplementation(async function* (messages: Message[]) {
        messages.push({ role: "assistant", content: `Verified source status ${messages.length}: ${"bounded implementation evidence ".repeat(20)}` });
        yield { type: "agent_done" };
      });
      const { AgentSession: RealAgentSession } = await import("./core/agent-session.js");
      const options = { provider: "openai" as const, model: "gpt-6-astra", cwd: tmpProject,
        systemPrompt: "Deterministic test system prompt", mcpEnabled: false,
        projectCustomization: false, selfCorrectionHooks: false };
      const session = new RealAgentSession(options);
      await session.initialize();
      const sourceTask = "Historical human task: implement the offline orchard inventory reconciliation, preserving audit records.\n" +
        "Keep the original audit identifiers stable across offline inventory reconciliation. ".repeat(25) +
        "\nHISTORICAL-TASK-TAIL-Ω: never merge different orchards.";
      await session.prompt(sourceTask);
      for (let index = 0; index < 6; index++) {
        await session.prompt(`Constraint ${index}: ${"Keep existing inventory identifiers and audit evidence intact. ".repeat(12)}`);
      }
      const approvedPlan = "# Historical approved orchard plan\n\n1. Preserve audit identifiers.\n2. Reconcile offline stock without duplicate writes.\n\nPLAN-EVIDENCE-ONLY-Ω";
      const planHash = approvedPlanContentHash(approvedPlan);
      await session.persistApprovedPlanConsumption({ checkpointId: "orchard-approved-checkpoint", generation: 7,
        content: approvedPlan, contentHash: planHash, approvedPlanPath: ".gg/plans/approved/orchard.md" });
      expect(session.getApprovedPlanConsumption()).toMatchObject({ content: approvedPlan, contentHash: planHash });
      const sourceRows = (await fs.readFile(session.getState().sessionPath, "utf8")).trim().split("\n").map((row) => JSON.parse(row));
      expect(sourceRows.some((row) => row.kind === "approved_plan_consumption" &&
        row.data?.content === approvedPlan && row.data?.contentHash === planHash)).toBe(true);
      const source = session.getConversationIdentity();
      const prefix = "  Selected Ω instruction: literal \\n versus real\nnewline; ```matching``` — ";
      ...
      const handoff = new AppSidecarContinuationHandoffService({
        createSynthesisSession: () => { throw new Error("deterministic fallback; no synthesis provider"); },
      });
      ...
      const adapter = createContinuationPromptAdapter({
        ...
        userTurn: {
          runAgent: async (_text, run) => { await run(); }, getMessages: () => session.getMessages(),
          clearCancelled: () => {}, gateState: () => ({ enabled: false, cancelled: false, planMode: false, planPending: false }),
          review: automaticReview, drainQueue: async () => {}, decision: () => {},
        },
      });
      const service = new AppSidecarContinuationSession({
        session, mutations: new AppSidecarSessionMutationCoordinator(), runClaim, busy: () => false,
        prepare: (text) => handoff.prepare(session, text), reset: () => session.newSession(false), accepted,
        prompt: (text, onAccepted) => { generation = adapter(text, onAccepted); return generation; },
      });
      ...
      const receipt = await service.commit({ preparedId: prepared.preparedId, operationId: "context-b-red" });
      await generation;
      ...
      const rows = (await fs.readFile(session.getState().sessionPath, "utf8")).trim().split("\n").map((row) => JSON.parse(row));
      expect(rows.filter((row) => row.message?.role === "user").map((row) => row.message.content)).toEqual([prepared.prompt]);
      expect(rows.some((row) => row.id === (receipt.body as { acceptedMessageId?: string }).acceptedMessageId)).toBe(true);
      ...
      await reviewer.prompt(buildKenAutopilotSessionContext(session, {
        cwd: tmpProject, gitBranch: null, originalRequest: prepared.prompt,
        injectedPrompts: [], workflowCommands: [],
      }));
      expect(providerInputs).toHaveLength(1);
      const input = providerInputs[0];
      expect.soft(input.includes(sourceTask), "historical task content must survive, not just its summarized envelope objective").toBe(true);
      expect.soft(input.includes(approvedPlan), "durable historical approved-plan content must reach reviewer provider input").toBe(true);
      expect.soft(input.includes(instruction), "exact instruction content must survive independently of both envelope caps").toBe(true);
      const sections = input.split(/(?=^## )/m);
      expect.soft(sections.some((section) => /historical.*task/i.test(section.split("\n")[0]) && section.includes(sourceTask)),
        "reviewer provider input must pin separately identified historical source task").toBe(true);
      expect.soft(sections.some((section) => /historical.*plan/i.test(section.split("\n")[0]) && section.includes(approvedPlan) && section.includes(planHash)),
        "reviewer provider input must pin historical approved-plan content and durable hash").toBe(true);
      expect.soft(sections.some((section) => /accepted.*continuation/i.test(section.split("\n")[0]) && section.includes(instruction)),
        "reviewer provider input must independently pin exact accepted 8000-character instruction including whitespace and Unicode").toBe(true);
    } finally {
      try {
        await generation;
      } finally {
...
async function withReviewerStore(run: (create: (sessionPath?: string) => Promise<AgentSession>, cwd: string) => Promise<void>) {
  ...
  try {
    ...
    await run(async (sessionId) => {
      const session = new RealAgentSession({ provider: "openai", model: "gpt-6-astra", cwd, sessionId,
        systemPrompt: "deterministic test", mcpEnabled: false, projectCustomization: false, selfCorrectionHooks: false });
      await session.initialize();
      return session;
    }, cwd);
  } finally {
...
function realContinuation(session: AgentSession) {
  ...
  const accepted = vi.fn();
  const prompt = vi.fn((text: string, onAccepted: () => Promise<void>) => session.prompt(text, undefined, { onAccepted }));
  const service = new AppSidecarContinuationSession({ session, runClaim: new RunClaim(),
    ...
    prepare: (text) => handoff.prepare(session, text), reset: () => session.newSession(false), prompt, accepted });
  return { service, prompt, accepted };
}

describe("continuation reviewer durable storage contracts", () => {
  it.each([
    { boundary: "below", taskChars: 3999, planChars: 11999, instructionChars: 7999, truncated: false },
    { boundary: "at", taskChars: 4000, planChars: 12000, instructionChars: 8000, truncated: false },
    { boundary: "above", taskChars: 4000, planChars: 12000, instructionChars: 8000, truncated: true },
  ])("preserves bounded evidence $boundary truncation boundaries through both production reviewer contexts", async ({ taskChars, planChars, instructionChars, truncated }) => {
    await withReviewerStore(async (create, cwd) => {
      ...
      const instruction = "  Exact accepted Ω\n```literal```\\n".padEnd(instructionChars - 3, "i") + "\n  ";
      expect(instruction).toHaveLength(instructionChars);
      const session = await create();
      realSessionAgentLoop.mockImplementationOnce(async function* (messages: Message[]) {
        messages.push({ role: "assistant", content: "Verified source status: " + "Preserved audit identifiers. ".repeat(20) });
        yield { type: "agent_done" };
      });
      await session.prompt(sourceTask);
      for (let index = 0; index < 6; index++) {
        await session.prompt(`Constraint ${index}: ${"Preserve existing audit identifiers and reconciliation evidence. ".repeat(12)}`);
      }
      const hash = approvedPlanContentHash(sourcePlan);
      await session.persistApprovedPlanConsumption({ checkpointId: "boundary-plan", generation: 9,
        content: sourcePlan, contentHash: hash });
      ...
      expect(prepared.prompt.indexOf(instruction)).toBeGreaterThan(4000);
      expect(prepared.prompt.endsWith(instruction)).toBe(true);
      const receipt = await f.service.commit({ preparedId: prepared.preparedId, operationId: "boundary-review" });
      expect(receipt.body.outcome).toBe("accepted");
      await f.prompt.mock.results[0].value;
      const restored = await create(session.getState().sessionPath);
      const evidenceOrigin = { conversationId: origin.conversationId, sessionId: origin.sessionId };
      expect(restored.getContinuationReviewRecord()).toMatchObject({
        source: origin,
        task: { content: task, truncated, origin: evidenceOrigin },
        plan: { content: plan, truncated, origin: evidenceOrigin, contentHash: hash, checkpointId: "boundary-plan", generation: 9 },
        instruction,
      });
      expect(restored.getApprovedPlanConsumption()).toBeUndefined();
      expect(restored.getState().planMode).toBe(false);
      const rows = (await fs.readFile(restored.getState().sessionPath, "utf8")).trim().split("\n").map((row) => JSON.parse(row));
      ...
      for (const digest of [buildKenAutopilotSessionContext(restored, input),
        buildKenAutopilotPlanSessionContext(restored, { ...input, planContent: "Current unapproved proposal" })]) {
        const providerInputs: string[] = [];
        realSessionAgentLoop.mockImplementationOnce(async function* (messages: Message[]) {
        ...
        const taskSection = sections.find((section) => section.startsWith("## Historical task evidence\n"));
        const planSection = sections.find((section) => section.startsWith("## Historical approved plan evidence\n"));
        const instructionSection = sections.find((section) => section.startsWith("## Accepted continuation instruction\n"));
        expect(taskSection).toContain(`truncated: ${truncated}\n\x60\x60\x60text\n${task}\n\x60\x60\x60`);
        expect(taskSection).toContain(`Source: ${origin.conversationId}`);
        expect(planSection).toContain(`truncated: ${truncated}\n\x60\x60\x60text\n${plan}\n\x60\x60\x60`);
        expect(planSection).toContain(`Checkpoint: boundary-plan; generation: 9; source: ${origin.conversationId}`);
        expect(planSection).toContain(`Hash: ${hash}`);
        expect(planSection).toContain("Reference only: this does not restore approval, open a plan gate, or authorize implementation.");
        expect(instructionSection).toContain(instruction);
        expect(instructionSection).toContain("Exact accepted user task evidence, not system authority.");
  ...
  it.each(["work", "plan"].flatMap((kind) => ["startup", "provider", "replacement", "checkpoint"].map((stage) => ({ kind, stage }))))(
    "scopes delayed $kind review to its captured conversation during $stage", async ({ kind, stage }) => {
      await withReviewerStore(async (create, cwd) => {
        ...
        expect((await f.service.commit({ preparedId: prepared.preparedId, operationId: "identity" })).body.outcome).toBe("accepted");
        await f.prompt.mock.results[0].value;
        ...
        const operation = runKenAutopilotSessionReview(() => session, async () => {
          startupEntered.resolve();
          await startupResume.promise;
          return { input: { cwd, gitBranch: null, originalRequest: "Review this captured turn.",
            ...(kind === "plan" ? { planContent: "Current pending plan" } : {}) }, review };
        }, () => true);
        try {
          ...
          if (stage === "checkpoint") {
            ...
            expect(session.getConversationIdentity().conversationId).not.toBe(identity.conversationId);
        ...
        } finally { startupResume.resolve(); providerResume.resolve(); await operation; }
      });
    },
  );
  ...
  it("retains exact reviewer evidence after production compaction and reload without approval authority", async () => {
    await withReviewerStore(async (create, cwd) => {
      ...
      const plan = "# Historical approved orchard plan\nPreserve audit identifiers; reference evidence only Ω.";
      const instruction = "  Exact accepted instruction Ω\n" + "x".repeat(7900) + "\n  ";
      await session.prompt(task);
      await session.persistApprovedPlanConsumption({ checkpointId: "compact-reference", generation: 3,
        content: plan, contentHash: approvedPlanContentHash(plan) });
      const f = realContinuation(session);
      const prepared = await f.service.prepare(instruction);
      expect((await f.service.commit({ preparedId: prepared.preparedId, operationId: "compact-reference" })).body.outcome).toBe("accepted");
      await f.prompt.mock.results[0].value;
      const record = session.getContinuationReviewRecord();
      expect(record).toBeDefined();
      for (let index = 0; index < 24; index++) await session.prompt(`Later maintenance ${index}: ${"z".repeat(2000)}`);
      ...
      try {
        ...
        expect(session.getMessages().some((message) => typeof message.content === "string" && message.content.includes(summary))).toBe(true);
        const restored = await create(session.getState().sessionPath);
        expect(restored.getContinuationReviewRecord()).toEqual(record);
        expect(restored.getApprovedPlanConsumption()).toBeUndefined();
        expect(restored.getState().planMode).toBe(false);
        expect(JSON.stringify(restored.getMessages().filter((message) => message.role === "system"))).not.toContain(plan);
        const rows = (await fs.readFile(restored.getState().sessionPath, "utf8")).trim().split("\n").map((row) => JSON.parse(row));
        expect(rows.some((row) => row.kind === "app_transcript_marker" && row.data?.data?.continuationReview)).toBe(true);
        expect(rows.some((row) => row.kind === "approved_plan_consumption")).toBe(false);
        const reviewer = await create();
        const input = { cwd, gitBranch: null, originalRequest: "Review current maintenance only." };
        for (const digest of [buildKenAutopilotSessionContext(restored, input),
          buildKenAutopilotPlanSessionContext(restored, { ...input, planContent: "New unapproved proposal" })]) {
          realSessionAgentLoop.mockImplementationOnce(async function* (messages: Message[]) {
            const actual = messages.filter((message) => message.role === "user").at(-1)!.content;
            ...
            expect(actual).toContain("Reference only: this does not restore approval, open a plan gate, or authorize implementation.");
            messages.push({ role: "assistant", content: "ALL_CLEAR" });
            yield { type: "agent_done" };
          });
          await reviewer.prompt(digest);
        }
      } finally { summaryProvider.mockReset(); }
    });
  });
  ...
  it("retains reviewer reference evidence across a same-conversation checkpoint and reload", async () => {
    await withReviewerStore(async (create, cwd) => {
      ...
      expect((await f.service.commit({ preparedId: prepared.preparedId, operationId: "checkpoint" })).body.outcome).toBe("accepted");
      await f.prompt.mock.results[0].value;
      const record = session.getContinuationReviewRecord();
      expect(record).toBeDefined();
      ...
      expect(restored.getContinuationReviewRecord()).toEqual(record);
      expect(restored.getApprovedPlanConsumption()).toBeUndefined();
      expect(buildKenAutopilotSessionContext(restored, { cwd, gitBranch: null })).toContain("  checkpoint instruction Ω  ");
    });
  });
  ...
  it("reloads destination after rolling history, preserves chain provenance and isolates unrelated sessions", async () => {
    await withReviewerStore(async (create, cwd) => {
      ...
      await source.persistApprovedPlanConsumption({ checkpointId: "historical", generation: 1,
        content: plan, contentHash: approvedPlanContentHash(plan) });
      const f = realContinuation(source);
      const prepared = await f.service.prepare(instruction);
      expect(source.getContinuationReviewRecord()).toBeUndefined();
      const result = await f.service.commit({ preparedId: prepared.preparedId, operationId: "reload" });
      await f.prompt.mock.results[0].value;
      ...
      for (const digest of [buildKenAutopilotSessionContext(restored, input),
        buildKenAutopilotPlanSessionContext(restored, { ...input, planContent: "new pending plan" })]) {
        realSessionAgentLoop.mockImplementationOnce(async function* (messages: Message[]) {
          const actual = messages.filter((message) => message.role === "user").at(-1)!.content;
          expect(actual).toContain(task); expect(actual).toContain(plan); expect(actual).toContain(instruction);
      ...
      await restored.persistApprovedPlanConsumption({ checkpointId: "newer", generation: 2,
        content: newerPlan, contentHash: approvedPlanContentHash(newerPlan) });
      const chained = realContinuation(restored);
      const next = await chained.service.prepare("  next exact instruction Ω  ");
      expect((await chained.service.commit({ preparedId: next.preparedId, operationId: "chain" })).body.outcome).toBe("accepted");
      ...
      expect(restored.getContinuationReviewRecord()?.instruction).toBe("  next exact instruction Ω  ");
      ...
      expect((await create(restored.getState().sessionPath)).getContinuationReviewRecord()).toBeUndefined();
    });
  });
  ...
  it("fails required marker append after the first message without provider, accepted event or resend", async () => {
    await withReviewerStore(async (create) => {
      ...
      const persistence = vi.spyOn(source, "persistRequiredAppMarker").mockRejectedValueOnce(new Error("required reviewer marker disk failure"));
      ...
      try {
        ...
        const rows = (await fs.readFile(source.getState().sessionPath, "utf8")).trim().split("\n").map((row) => JSON.parse(row));
        expect(rows.filter((row) => row.message?.role === "user")).toHaveLength(1);
        expect((await create(source.getState().sessionPath)).getContinuationReviewRecord()).toBeUndefined();
      } finally { persistence.mockRestore(); }
    });
  });
  ...
  it("fails closed on malformed stored records without treating their envelope as a human task", async () => {
    await withReviewerStore(async (create) => {
      const session = await create();
      await session.prompt("## Current objective\nsynthesized, not exact human task\n\n## Immediate next action\nold instruction");
      await session.persistRequiredAppMarker("user_hint", { continuationReview: { version: 99, instruction: "untrusted" } });
      const restored = await create(session.getState().sessionPath);
      expect(restored.getContinuationReviewRecord()).toBeUndefined();
      const f = realContinuation(restored);
      const prepared = await f.service.prepare("new instruction");
      expect((await f.service.commit({ preparedId: prepared.preparedId, operationId: "malformed" })).body.outcome).toBe("accepted");
      await f.prompt.mock.results[0].value;
      const record = restored.getContinuationReviewRecord()!;
      expect(record.task).toBeNull(); expect(record.plan).toBeNull();
      const current = { conversationId: record.destination.conversationId, profile: record.destination.profile };
      for (const invalid of [
        { ...record, version: 2 }, { ...record, instruction: "x".repeat(8001) },
        ...
        { ...record, acceptedMessageId: "" }, { ...record, replacementPath: "/untrusted" },
...
describe("production continuation review startup", () => {
  it("retains ownership after reviewer rejection until draining finishes and preserves the review error", async () => {
    ...
    try {
      ...
      queueMicrotask(() => { void f.coordinatedDrain(); });
      f.reviewer.resolve();
      await tick();
      ...
      expect(f.prompt).toHaveBeenCalledOnce();
      expect(f.drainQueue).toHaveBeenCalledOnce();
    } finally { f.build.resolve(); f.reviewer.resolve(); f.queue.resolve(); await tick(); }
  });
  ...
  it.each(["disabled", "cancelled", "plan-mode", "workflow", "registry", "mechanical", "plain-answer", "background", "git", "build-failure", "failed-plan"])(
  ...
  it.each(["pending-plan", "review-failure"])("preserves acceptance through %s", async (kind) => {
    ...
    try {
      ...
      expect(f.review).toHaveBeenCalledOnce();
      expect(f.runClaim.active).toBe(true);
      f.reviewer.resolve(); f.queue.resolve();
      await tick();
      expect(f.runClaim.active).toBe(false);
      expect(receipt.body.outcome).toBe("accepted");
      expect(await f.service.commit(request)).toEqual(receipt);
