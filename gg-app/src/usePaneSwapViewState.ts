...
export interface PaneSwapViewState { capture: () => (() => void) }
...
function readingAnchor(container: HTMLElement): { range: Range | null; node: Node; offset: number } | null {
  const viewport = container.getBoundingClientRect();
  const top = viewport.top + container.clientTop;
  const bottom = top + container.clientHeight;
  const intersects = (rect: DOMRect) => rect.width > 0 && rect.height > 0 && rect.bottom > top && rect.top < bottom;
  for (const row of container.querySelectorAll<HTMLElement>("[data-swap-row]")) {
    ...
    while ((node = walker.nextNode())) {
      ...
      if (!length || !node.parentElement || getComputedStyle(node.parentElement).visibility !== "visible") continue;
    ...
    if (row.querySelector("img")) return { range: null, node: row, offset: rect.top - viewport.top };
  }
  return null;
}

export function usePaneSwapViewState({ paneId, scrollRef, inputRef, stickToBottomRef, register }: Options) {
  ...
  useLayoutEffect(() => {
    const cancel = () => { generation.current++; restoredScroll.current = null; };
    ...
    const capture = () => {
      ...
      const selection = input ? { value: input.value, start: input.selectionStart, end: input.selectionEnd, direction: input.selectionDirection } : null;
      ...
      return () => {
        ...
        if (pinned) target = el.scrollHeight - el.clientHeight;
          ...
          const rect = anchor.range?.getBoundingClientRect() ?? (anchor.node as Element).getBoundingClientRect();
          if (rect.width > 0 && rect.height > 0 &&
              (!anchor.range || (!anchor.range.collapsed && anchor.range.startContainer === anchor.node)) &&
              getComputedStyle(anchor.node instanceof Element ? anchor.node : anchor.node.parentElement!).visibility === "visible") {
        ...
        if (input?.isConnected && selection && input.value === selection.value &&
            input.selectionStart === selection.start && input.selectionEnd === selection.end) {
