...
it.each([-1, 0, 100, "legacy"] as const)(
  "routes matching RPC/event cap metadata (%s) to its ready secondary without primary readiness",
  async (extra) => {
    ...
    invoke.mockImplementation(async (command: string, args: { paneId: string }) => {
      ...
      if (command === "agent_pane_status")
        return { ready: true, error: null, generation: 2, sessionId: "right-current" };
    ...
    try {
      ...
      const send = (paneId: string, sessionId: string, origin: boolean) =>
