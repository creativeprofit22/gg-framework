...
function fixture(pinned = false) {
  const { container } = render(
    <>
      <div data-testid="scroll">
        <p data-swap-row="1">A long paragraph for reading</p>
      </div>
      <textarea defaultValue="unfinished draft" />
    </>,
  );
  ...
  const register = (_id: string, next: PaneSwapViewState | null) => {
    state = next;
  };
  const refs = { scrollRef: { current: scroll }, inputRef: { current: input } };
  const hook = renderHook(() =>
    usePaneSwapViewState({ paneId: "a", ...refs, stickToBottomRef: pin, register }),
  );
...
describe("pane-local swap view state", () => {
  it("retains drafts, caret and bottom-pin streaming intent without focusing", () => {
    ...
    f.input.setSelectionRange(2, 7, "backward");
    const restore = f.capture();
    Object.defineProperty(f.scroll, "scrollHeight", { value: 1200 });
    act(restore);
    expect(f.scroll.scrollTop).toBe(1000);
    expect(f.pin.current).toBe(true);
    expect(f.input.value).toBe("unfinished draft");
    expect([f.input.selectionStart, f.input.selectionEnd, f.input.selectionDirection]).toEqual([
      2,
      7,
      "backward",
    ]);
  ...
  it("uses a visible character anchor through reflow, retaining unpinned intent", () => {
    const f = fixture();
    let reflow = 0;
    vi.spyOn(f.scroll.querySelector("p")!, "getBoundingClientRect").mockReturnValue({
      top: -30,
      bottom: 400,
      width: 200,
      height: 430,
    ...
    try {
      ...
      reflow = 60;
      act(restore);
      expect(f.scroll.scrollTop).toBe(160);
      expect(f.pin.current).toBe(false);
    } finally {
  ...
  it.each(["offscreen", "hidden"])(
    "does not anchor a later %s row across a missing-anchor gap",
    (kind) => {
      const f = fixture();
      vi.spyOn(f.scroll, "getBoundingClientRect").mockReturnValue({
        top: 0,
        bottom: 200,
      } as DOMRect);
      ...
      Object.defineProperty(Range.prototype, "getBoundingClientRect", {
        configurable: true,
        value: () => ({ top, bottom: top + 20, width: 100, height: 20 }),
      });
      try {
        const restore = f.capture();
        top += 120;
  ...
  it("falls back to a clamped offset and ignores superseded or user-cancelled restores", () => {
    ...
    f.scroll.scrollTop = 250;
    act(stale);
    expect(f.scroll.scrollTop).toBe(250);
    act(current);
    expect(f.scroll.scrollTop).toBe(100);
    const cancelled = f.capture();
    fireEvent.wheel(f.scroll);
    f.scroll.scrollTop = 300;
    act(cancelled);
    expect(f.scroll.scrollTop).toBe(300);
    f.hook.unmount();
    expect(f.getState()).toBeNull();
  });
});
