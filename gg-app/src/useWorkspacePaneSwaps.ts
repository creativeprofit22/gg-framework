...
const modalSelector = '.modal-backdrop, .menu-backdrop, .slash-menu, .dropdown-menu, .bgtasks-menu, [role="dialog"], [role="alertdialog"], [role="menu"], [role="listbox"]';
interface Options {
...
export function useWorkspacePaneSwaps({ layout, layoutRef, gridRef, closingPaneIdsRef, commit, focusPane, label }: Options) {
  ...
  const registerHost = useCallback((id: string, el: HTMLElement | null) => {
    if (el) hosts.current.set(id, el); else hosts.current.delete(id);
  }, []);
  const registerButton = useCallback((id: string, el: HTMLButtonElement | null) => {
    if (el) buttons.current.set(id, el); else buttons.current.delete(id);
  }, []);
  const registerViewState = useCallback((id: string, state: PaneSwapViewState | null) => {
    if (state) views.current.set(id, state); else views.current.delete(id);
  }, []);
  const preserveFocus = useCallback(() => focusRoot.current === layoutRef.current.root ||
    [...buttons.current.values()].some((button) => button === document.activeElement), [layoutRef]);
  useEffect(() => {
    const release = (event: PointerEvent) => {
      clearKeyboardFocus();
      if (!(event.target instanceof Element) || !event.target.closest("[data-pane-swap]")) focusRoot.current = null;
    };
  ...
  const rows = new Map(workspacePaneRects(layout.root, size).map((r, _index, all) => [r.paneId, findSwapRow(all, r.paneId)]));

  const swap = useCallback((side: string, buttonActivation = true, keyboard = false, centerDirection?: "left" | "right"): boolean => {
    const grid = gridRef.current;
    ...
    if (!row.available) { setAnnouncement(row.reason); return false; }
    ...
    const activeDirection = active instanceof HTMLElement && active.dataset.paneSwap === row.middleId
      ? active.dataset.paneSwapDirection : undefined;
    const direction = centerDirection ?? (activeDirection === "left" || activeDirection === "right" ? activeDirection : undefined);
    const handoff = buttonActivation || active === buttons.current.get(side) || Boolean(direction);
    ...
    const restores = [side, row.middleId].flatMap((id) => {
      const view = views.current.get(id); return view ? [view.capture()] : [];
    });
    ...
    flushSync(() => {
      ...
      pending.current = { generation: ++generation.current, side, middle: row.middleId, targetKey, focusId, origin, active, handoff, keyboard, restores, root: next.root, before };
      accepted = true;
      setAnnouncement(`${label(side)} moved into the middle.${handoff ? direction ? " The focused center action can exchange the pair again." : " The focused side action can exchange the pair again." : ""}`);
    });
    return accepted;
  }, [gridRef, layoutRef, closingPaneIdsRef, commit, label, cancelAnimations]);

  const swapFromMiddle = useCallback((middle: string, direction: "left" | "right", keyboard: boolean) => {
    const grid = gridRef.current;
    if (!grid) return false;
    const row = findSwapRow(workspacePaneRects(layoutRef.current.root, grid.getBoundingClientRect()), middle);
  ...
  useLayoutEffect(() => {
    ...
    if (!job) { cancelAnimations(); return; }
    pending.current = null;
    if (job.generation !== generation.current) return;
    for (const restore of job.restores) restore();
    if (job.root === layout.root && !document.hidden && !window.matchMedia?.("(prefers-reduced-motion: reduce)").matches) {
    ...
    const active = document.activeElement;
    if (document.querySelector(modalSelector) || (active !== job.active && active !== document.body && active?.isConnected)) return;
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.defaultPrevented || event.isComposing || event.repeat || event.getModifierState("AltGraph") ||
          !event.altKey || !event.shiftKey || (mac ? !event.metaKey || event.ctrlKey : !event.ctrlKey || event.metaKey) ||
          (event.key !== "ArrowLeft" && event.key !== "ArrowRight")) return;
      const active = document.activeElement;
      const grid = gridRef.current;
      if (!(active instanceof Element) || !grid?.contains(active) || document.querySelector(modalSelector)) return;
      const id = active.closest<HTMLElement>("[data-pane-id]")?.dataset.paneId;
      if (!id) return;
      const row = findSwapRow(workspacePaneRects(layoutRef.current.root, grid.getBoundingClientRect()), id);
      if (!row.available) { setAnnouncement(row.reason); return; }
  ...
  useEffect(() => {
    ...
    const hidden = () => { if (document.hidden) cancelAnimations(); };
    preference?.addEventListener("change", cancelAnimations);
    document.addEventListener("visibilitychange", hidden);
    return () => {
      generation.current++; pending.current = null; cancelAnimations();
  ...
  const shortcutHelp = /Mac|iPhone|iPad/.test(navigator.platform)
    ? "Cmd+Option+Shift+Left or Right" : "Ctrl+Alt+Shift+Left or Right";
  return { rows, swap, swapFromMiddle, registerHost, registerButton, registerViewState, preserveFocus, announcement,
