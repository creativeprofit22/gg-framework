...
import { findSwapRow, swapWorkspacePanes, workspacePaneRects, type PaneRect } from "./workspace-swaps";
import { WORKSPACE_LAYOUT_VERSION, workspaceLayoutLeafIds, type WorkspaceLayout, type WorkspaceLayoutNode } from "./workspace-layout";
const size = { width: 1600, height: 900 };
const leaf = (paneId: string): WorkspaceLayoutNode => ({ type: "leaf", paneId });
const split = (first: WorkspaceLayoutNode, second: WorkspaceLayoutNode, direction: "horizontal" | "vertical" = "horizontal", ratio = 50): WorkspaceLayoutNode => ({ type: "split", direction, ratio, size: { type: "ratio", value: ratio }, first, second });
const row = (ids: string[]): WorkspaceLayoutNode => ids.length === 1 ? leaf(ids[0]) : split(leaf(ids[0]), row(ids.slice(1)), "horizontal", 100 / ids.length);
const layout = (root: WorkspaceLayoutNode): WorkspaceLayout => ({ version: WORKSPACE_LAYOUT_VERSION, root, focusedPaneId: "primary", panes: Object.fromEntries(workspaceLayoutLeafIds(root).map((id) => [id, null])) });
const rects = (widths: number[]): PaneRect[] => { let left = 0; return widths.map((width, i) => { const r = { paneId: `${i}`, left, top: 0, width, height: 100 }; left += width + 7; return r; }); };
describe("physical same-row middle swaps", () => {
  it.each([3, 4, 5, 6])("targets %i aligned columns, including farther sides", (count) => {
    ...
    for (const r of rs) expect(findSwapRow(rs, r.paneId)).toEqual({ available: true, paneIds: rs.map((p) => p.paneId), middleId: `${Math.floor((count - 1) / 2)}` });
  });
  ...
  it("never crosses a spanning pane, and permits an unambiguous partial row", () => {
    const rs = rects([100, 100, 100, 100]); rs[0].height = 207;
  ...
  it("rejects transitive boundary chains and hidden geometry", () => {
    const rs = rects([100, 100, 100]); rs[0].top = -0.8; rs[2].top = 0.8;
    expect(findSwapRow(rs, "1").available).toBe(false);
    expect(findSwapRow(rects([100, 0, 100]), "0").available).toBe(false);
  });
  it.each(["a", "c", "d", "f"])("exchanges %s in either row, preserving everything else", (side) => {
    ...
    expect(workspaceLayoutLeafIds(changed.root)).toEqual(workspaceLayoutLeafIds(original.root).map((id) => id === side ? middle : id === middle ? side : id));
    if (changed.root.type === "split" && original.root.type === "split") {
      ...
      expect(middle === "b" ? changed.root.second : changed.root.first).toBe(middle === "b" ? original.root.second : original.root.first);
    }
  ...
  it("guards stale middle targets, closing/splitting and primary identity", () => {
    ...
    expect(changed.focusedPaneId).toBe("primary"); expect(changed.panes).toBe(original.panes);
