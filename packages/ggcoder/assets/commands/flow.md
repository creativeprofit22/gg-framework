...
**Ground unfamiliar framework patterns with Steroids before classifying.** When a flow depends on a framework hook, IPC pattern, store API, router behaviour, or event-bus convention you're not certain about, use **Steroids** to look up how the pattern is wired in real public apps *before* calling something BROKEN, STALE, or ASYMMETRIC. Two reasons:
...
- **Fallback grounding clause**: if your Step 4 recipe is ambiguous or you couldn't verify the pattern (rare framework, no public examples), tell the fix agent to search the curated Steroids corpus for the specific literal symbol and inspect matches with `show` *before* writing code. If Steroids reports a real corpus gap, tell it to `discover`, seek approval before `add`, then repeat `search` and `show`. Otherwise omit — don't pad every task with redundant lookup instructions.
