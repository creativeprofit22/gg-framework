...
const sourceSession = {
  getState: () => ({ provider: "anthropic" as const, model: "test", cwd: "/repo" }),
};
...
describe("decision summary route", () => {
  it("returns only the validated summary envelope", async () => {
    const summarize = vi.fn(async () => ({
      version: 1 as const,
      summary: "A useful summary that is comfortably longer than forty characters.",
    }));
    const response = await handleDecisionSummaryRequest(JSON.stringify(context), sourceSession, {
      summarize,
    });
  ...
  it.each(["", "{bad-json", JSON.stringify({ ...context, decisions: [] })])(
    ...
    async (raw) => {
      ...
      expect((await handleDecisionSummaryRequest(raw, sourceSession, { summarize })).status).toBe(
        400,
      );
  ...
  it("rejects oversized input before model creation", async () => {
    const summarize = vi.fn();
    const response = await handleDecisionSummaryRequest(
      "x".repeat(DECISION_SUMMARY_CONTEXT_MAX_BYTES + 1),
      sourceSession,
      { summarize },
    );
  ...
  it("does not echo evidence when synthesis fails", async () => {
    const summarize = vi.fn(async () => {
      throw new Error("provider saw secret-source-text");
    });
    const response = await handleDecisionSummaryRequest(JSON.stringify(context), sourceSession, {
      summarize,
    });
