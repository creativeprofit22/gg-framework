...
describe("useKenMentor", () => {
  it("captures copied idle target authority and isolates pending operations from another pane reset", () => {
    ...
    act(() => b.hook.result.current.hydrateKen({ ...identity, activationEpoch: "rewound", activeRunId: null }, true));
  ...
  it("never revives pending operations after a same-conversation epoch changes and returns", () => {
    ...
    act(() => {
      hook.result.current.hydrateKen({ ...identity, activationEpoch: "rewound", activeRunId: identity.runId }, true);
  ...
  it.each([null, undefined, "bad", 42, false, []].map((data) => ({ data })))("consumes malformed mentor payload $data without changing state", ({ data }) => {
    const { hook, getItems } = setup();
    for (const type of ["ken_run_start", "ken_text_delta", "ken_thinking_delta", "ken_tool_call_start", "ken_tool_call_end", "ken_run_end", "ken_error"]) {
      act(() => expect(hook.result.current.handleKenEvent({ type, data } as SidecarEvent)).toBe(true));
    }
    expect(getItems()).toEqual([]);
    expect(hook.result.current.captureKenRun()).toEqual(identity);
    expect(hook.result.current.kenRunning).toBe(true);
    expect(hook.result.current.handleKenEvent({ type: "text_delta", data } as SidecarEvent)).toBe(false);
  });
  it("uses ready authority on reconnect and clears missing authority without learning from a model change", () => {
    const { hook, getItems } = setup();
    act(() => hook.result.current.handleKenEvent(ev("ready", { kenState: { ...identity, activeRunId: "reconnected" } })));
    act(() => hook.result.current.handleKenEvent(ev("ken_text_delta", { ken: { ...identity, runId: "reconnected" }, text: "resumed" })));
    ...
    act(() => hook.result.current.handleKenEvent(ev("ken_run_start", { ken: { ...identity, runId: "reconnected" } })));
  ...
  it.each(["ken_run_start", "ken_text_delta", "ken_thinking_delta", "ken_tool_call_start", "ken_tool_call_update", "ken_tool_call_end", "ken_server_tool_call", "ken_turn_end", "ken_run_end", "ken_error"])("consumes stale %s without disturbing the current run", (type) => {
    const { hook, getItems } = setup();
    act(() => hook.result.current.handleKenEvent(ev("ken_text_delta", { text: "current" })));
    act(() => expect(hook.result.current.handleKenEvent(ev(type, { ken: { ...identity, runId: "old" }, text: "OLD", message: "OLD", usage: { outputTokens: 99 } }))).toBe(true));
  ...
  it("never learns authority from a start, model or draft and never revives a closed run", () => {
    ...
    act(() => hook.result.current.handleKenEvent(ev("ken_run_end", { ken: { ...identity, runId: "next" } })));
    act(() => {
      hook.result.current.hydrateKen({ ...identity, activeRunId: "next" });
      hook.result.current.handleKenEvent(ev("ken_run_start", { ken: { ...identity, runId: "next" } }));
    });
    expect(hook.result.current.kenRunning).toBe(false);
  });
  it("guards pending initial hydration after reset and isolates two panes", () => {
    ...
    act(() => {
      a.hook.result.current.hydrateKen({ conversationId: "new", activationEpoch: "new", activeRunId: null }, true);
  ...
  it("retains streaming across checkpoint extras but breaks it on history replacement", () => {
    ...
    act(() => {
      ...
      hook.result.current.handleKenEvent(ev("extras", { kenState: { ...identity, activeRunId: identity.runId } }));
      hook.result.current.handleKenEvent(ev("ken_text_delta", { text: " retained" }));
    });
    expect(getItems()).toMatchObject([{ text: "before retained" }]);
    act(() => {
      hook.result.current.hydrateKen({ ...identity, activationEpoch: "replacement", activeRunId: "replacement-run" }, true);
      hook.result.current.handleKenEvent(ev("ken_text_delta", { ken: { ...identity, activationEpoch: "replacement", runId: "replacement-run" }, text: "after" }));
    });
    expect(getItems()).toMatchObject([{ text: "before retained" }, { text: "after" }]);
  });
  it.each(["ken_run_start", "ken_text_delta", "ken_thinking_delta", "ken_tool_call_start", "ken_tool_call_end", "ken_run_end", "ken_error"])("consumes missing metadata on %s without changing mentor state", (type) => {
    const { hook, getItems } = setup();
    act(() => { expect(hook.result.current.handleKenEvent({ type, data: { text: "stale" } } as SidecarEvent)).toBe(true); });
    expect(getItems()).toEqual([]);
    expect(hook.result.current.captureKenRun()).toEqual(identity);
    expect(hook.result.current.kenRunning).toBe(true);
  });
  ...
  it.each(["ken_tool_call_start", "ken_server_tool_call"])("handles current thinking, %s and end without joining pre-tool text", (type) => {
    const { hook, getItems } = setup();
    act(() => {
      hook.result.current.handleKenEvent(ev("ken_run_start"));
