...
function renderProgrammaticPrompt(scanRegistered: boolean): string {
  const steps = [
    ...
    "Submit programmatic_advisory_result with the version=2 kind=advisory schema, coverage and at most 10 recommendations, at most 64,000 serialized characters. Empty recommendations are valid. Use its validated presentation as Recommendations — not started, including coverage limitations. Do not write a second full report repeating rendered accepted advice. Follow with only a brief user-facing outcome, any limits not already visible, and the next permitted action. Available reuse choices require the exact resolved host snapshot and local prerequisite receipts; unavailable choices state why. Do not run recommended commands.",
...
3. Keep the exact inventory, configuration fingerprint, profile, routes, exclusions, drift inputs and fixed profile path unchanged in tool data and approval review; do not recite these raw details in the setup transcript. Give a brief user-facing outcome, inspection limits and next permitted action instead. Do not repeat rendered accepted advice.
...
6. When setup is current, briefly explain what its saved settings cover without proposing regeneration or approval. Source and lifecycle changes only need an explicit rescan, not new setup.
7. For a refresh, retain the exact added, removed and modified configuration inputs and policy, schema or exclusion differences in tool data and approval review; briefly explain their decision-critical effects in the transcript. A known legacy schema has no prior per-file baseline: explain that limitation and retain all current inputs in the review, never invent historical changes. Unreadable or unsupported data requires repair, not an overwrite.
8. Only when approval is available, briefly explain the separate approval needed to save settings. Keep this exact tool contract in tool data and approval review, not a transcript recital: a later separately approved invocation must call the \`generate\` action with the exact returned fingerprint, profile and \`expected_prior_profile_digest\` (including null for missing setup). The digest is a concurrency precondition, not user approval. Saving refreshed settings never scans: a later explicit Run project checks reconciles old results and lifecycle history.`,
  },
  {
    name: "programmatic",
    aliases: [],
...
4. Make each task prompt standalone: include the finding type (MISSING, DIVERGENT, or INCOMPLETE), exact file and line, the local implementation, the multi-repo Steroids evidence, and the concrete correction. The task must be actionable by an agent with no conversation context.
