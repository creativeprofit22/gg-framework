...
import {
  ...
  MAX_LEVEL,
  xpForLevel,
} from "../../packages/ggcoder/src/core/progress/ranks";
...
describe("rank cap presentation", () => {
  it("keeps ordinary progress below the authoritative cap", () => {
    ...
    const { container } = render(
      <>
        <RankBadge snapshot={snapshot} onClick={() => {}} />
        <ScorecardModal snapshot={snapshot} onClose={() => {}} />
      </>,
    );
    expect((container.querySelector(".rank-badge") as HTMLButtonElement).title).toContain(
      "3560/3561 XP to next",
    );
    expect(document.querySelector(".scorecard-level-row")?.textContent).toContain(
      `${new Intl.NumberFormat().format(3560)} / ${new Intl.NumberFormat().format(3561)} XP · 99%`,
    );
  ...
  it.each([1, 50, 1000])("does not infer a legacy cap from level %i or its sentinel", (level) => {
    ...
    const { container } = render(
      <>
        <RankBadge snapshot={legacy} onClick={() => {}} />
        <ScorecardModal snapshot={legacy} onClose={() => {}} />
      </>,
    );
    ...
    expect(
      screen.getByText(`${new Intl.NumberFormat().format(legacy.xp)} lifetime XP`),
    ).toBeTruthy();
  ...
  it.each([0, 100])("shows maximum level at cap + %i XP without a next-level fraction", (extra) => {
    ...
    expect(screen.getByText("Maximum level · 100%")).toBeTruthy();
    expect(
      screen.getByText(`${new Intl.NumberFormat().format(snapshot.xp)} lifetime XP`),
    ).toBeTruthy();
